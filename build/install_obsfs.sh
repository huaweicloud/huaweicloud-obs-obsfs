...
function install_obsfs()
{
    ...
    if [ ! -d "${OBSFS_LOG_PATH}" ]; then
        mkdir $OBSFS_LOG_PATH
    fi
    chmod 750 $OBSFS_LOG_PATH
}
...
function install_log_rotate()
{
    ...
    if [ ! -d ${log_rotate_shell_path} ]; then
        mkdir -p -m 750 ${log_rotate_shell_path}
    fi
    ...
    # check crontab job
    result=$(crontab -l 2>/dev/null| grep obsfs_log_rotate.sh)
    if [ -n "${result}" ]; then
        log "Crontab job for obsfs_log_rotate.sh already exists, skip adding."
        return
    fi
    ...
    if [ -n "${is_support_crontab}" ]; then
        cmd_log_rotate="[ -f /opt/dfv/obsfs/obsfs_log_rotate.sh ] && bash /opt/dfv/obsfs/obsfs_log_rotate.sh"
        (crontab -l 2>/dev/null;echo "*/10 * * * *  ${cmd_log_rotate}") | crontab -
...
function install_obsfsconfig()
{
    ...
    case ${mode} in
        --default)
            ...
            log "Create obsfsconfig file for default scene success."
            ;;        

        --gene)
            install_gene_obsfsconfig
            log "Create obsfsconfig file for gene scene success."
            ;;  

        --hpc)
            install_hpc_obsfsconfig
            log "Create obsfsconfig file for hpc scene success."
            ;;
