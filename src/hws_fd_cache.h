...
extern std::atomic<off64_t> g_wholeCacheTotalMemSize;  // used by ClearWholeCache wrapper

extern void hws_cache_daemon_task();

// Write/read proc functions (defined in hws_s3fs.cpp)
...
extern int s3fs_write_object_proc(const char* path, const char* buf, size_t size, off_t offset);
...
struct HwsFdWritePage
{
    ...
    const char* GetDataBuf() const  // const: no production code modifies via this accessor
    {
        return data_;
    }
    void WritePageRef()
...
class HwsFdEntity
{
  private:
    ...
    off64_t fileSize_;
    // [ISSUE2026040800004] mutable: required so GetEntityMutex() can be const.
    // The mutex is the synchronization primitive shared with HwsFdWritePageList,
    // HwsFdReadPageList, HwsFdReadPage, and external sync participants like
    // whole_file_cache_task — locking it does not modify the entity's logical
    // state, so it should be callable from a const context. This is the
    // standard C++ idiom for thread-safe class mutex members
    // (Effective Modern C++ Item 16; C++ Core Guidelines CON.4).
    mutable std::mutex entity_mutex_;        /*for write and read page list lock*/
  ...
  public:
    ...
    const std::string& GetPath() const  // const: callers only read path_
    {
        return path_;
    }
    // [ISSUE2026040800004] const: returns the synchronization primitive shared
    // between this entity and its read/write sub-objects. It does not modify
    // entity state. Pairs with mutable on entity_mutex_ above.
    std::mutex& GetEntityMutex() const
    {
        return entity_mutex_;
    }
    // Wrapper methods for wholeCachePage_ (replaces GetReadWholePageList)
    void AddWholeCachePage(HwsFdReadPage* page)
    {
        wholeCachePage_.pages_.push_back(page);
    }
    void SetWholeCacheEndTime()  // captures current monotonic time
    {
        clock_gettime(CLOCK_MONOTONIC_COARSE, &wholeCachePage_.cacheEndTs);
    }
    const struct timespec& GetWholeCacheEndTs() const
    {
        return wholeCachePage_.cacheEndTs;
    }
    off64_t GetWholeCacheHitCount() const
    {
        return wholeCachePage_.cacheHitCnt;
    }
    void ClearWholeCache()  // release all pages, update global mem counter, reset hit count
    {
        auto iter = wholeCachePage_.pages_.begin();
        while(iter != wholeCachePage_.pages_.end())
        {
            ...
            do
            {
                if(page->CanRelease())
                {
                    iter = wholeCachePage_.pages_.erase(iter);
    ...
    int Flush();
    // Check if entity has pending write pages that need flushing
    // Used by flush/fsync to skip unnecessary flush calls
    // Thread-safe: acquires entity_mutex_ internally
...
class HwsFdManager
{
  ...
  public:
    ...
    std::shared_ptr<HwsFdEntity> GetByPath(const std::string& path);
    bool Close(const uint64_t inodeNo);

    // Get all open file entities (for debugging and global flush)
    // Returns a snapshot of all currently open file descriptors
    // Thread-safe: locks internal mutex during iteration
    std::vector<std::shared_ptr<HwsFdEntity>> GetAllEntities();

    // Get all open file entities under a specific path (for directory rename)
    // For example, GetAllEntitiesUnderPath("/dir") returns all open files under /dir/*
    // Returns a snapshot of matching entities
    // Thread-safe: locks internal mutex during iteration
    std::vector<std::shared_ptr<HwsFdEntity>> GetAllEntitiesUnderPath(const std::string& path);
...
// Utility function for error type string conversion
