...
size_t S3fsCurl::ReadUploadCache(void* ptr, size_t blockSize, size_t numBlocks, void* data)
{
    ...
    {
        S3FS_PRN_ERR("CurlRest: User buffer has no Data need to update, bufersize:(%lu), readIndex:(%lu), size=(%ld), nmemb=(%ld)",
                databuff->ulBufferSize, databuff->offset, blockSize, numBlocks);
        return 0;
    }
...
size_t S3fsCurl::DownloadWriteCallbackforRead(void* ptr, size_t size, size_t nmemb, void* userp)
{
  ...
  S3FS_PRN_DBG("[readSize=%zd][dataSize=%zd][cur_pos=%zd]", pCurl->read_size,
...
std::string S3fsCurl::PrintRequestId4ResponseErr()
{
  ...
  {
    S3FS_PRN_INFO("curl perform failed,url=%s,response=%lu,requestid=%s",
        url.c_str(),LastResponseCode,requestIdStr.c_str());
  }
  else if (logflag)
  {
    S3FS_PRN_ERR("curl perform failed,url=%s,response=%lu,requestid=%s",
        url.c_str(),LastResponseCode,requestIdStr.c_str());
  }
...
/* file gateway modify begin */
      ...
      S3FS_PRN_DBG("[payload_hash=%s],[buf=%s]", payload_hash.c_str(), writedatabuf.pcBuffer);
/* file gateway modify end */
      break;
...
bool S3fsCurl::PreHeadRequest(const char* tpath, const char* bpath, const char* savedpath, int ssekey_pos, long long *inode_no, const char* shardkey)
{
    ...
    // create_handle_tv;
    //s3fsStatisStart(PRE_HEAD_CUR_HANDLE_DELAY, &create_handle_tv);
    if(!CreateCurlHandle(true)){
      return false;
    }
    //s3fsStatisEnd(PRE_HEAD_CUR_HANDLE_DELAY, &create_handle_tv);

    string resource;
    string turl;
    //timeval create_makeurl_tv;
    //s3fsStatisStart(PRE_HEAD_MAKE_CURL_DELAY, &create_makeurl_tv);
    MakeUrlResource(get_realpath(tpath).c_str(), resource, turl);
    ...
    //s3fsStatisEnd(PRE_HEAD_MAKE_CURL_DELAY, &create_makeurl_tv);

    //timeval auth_tv;
    //s3fsStatisStart(PRE_HEAD_AUTH_DELAY, &auth_tv);

    op = "HEAD";
    type = REQTYPE_HEAD;
    insertAuthHeaders();

    //s3fsStatisEnd(PRE_HEAD_AUTH_DELAY, &auth_tv);
...
/* file gateway modify end */
    ...
    curl_easy_setopt(hCurl, CURLOPT_HEADERFUNCTION, HeaderCallback);
    S3fsCurl::AddUserAgent(hCurl);                   // put User-Agent
    //s3fsStatisEnd(PRE_HEAD_REQUST_DELAY, &begin_tv);
    return true;
}
/* file gateway modify end */
...
int S3fsCurl::HeadRequest(const char* tpath, headers_t& meta, long long* inode_no, const char* shardkey)
{
  ...
  S3FS_PRN_INFO3("[tpath=%s]", SAFESTRPTR(tpath));
  //timeval beginHead_tv;
  //timeval match_begin_tv;

  //s3fsStatisStart(HEAD_DELAY, &beginHead_tv);
  //s3fsStatisStart(HEAD_REQUEST_MISS_DELAY, &miss_begin_tv);
  ...
  if (!Preflag){
    S3FS_PRN_ERR("PreHeadRequest failed, inde_no = %lld, shardkey = %s", *inode_no, shardkey);
    return -1;
  }
                                     ...
                                     tag_index_cache_entry_t *fileMeta)
{
    ...
    //s3fsStatisEnd(WRITE_CUR_HANDLE_DELAY, &create_handle_tv);
    ...
    // is meta necessary ??
    /* todo add meta */
    //s3fsStatisEnd(WRITE_MAKE_CURL_DELAY, &begin_tv);

    op = "PUT";
    type = REQTYPE_PUT;
    //s3fsStatisStart(WRITE_AUTH_DELAY, &begin_tv);
    insertAuthHeaders();
    //s3fsStatisEnd(WRITE_AUTH_DELAY, &begin_tv);
    // setopt
    curl_easy_setopt(hCurl, CURLOPT_URL, url.c_str());
    curl_easy_setopt(hCurl, CURLOPT_UPLOAD, true);                // HTTP PUT
    ...
    S3FS_PRN_INFO3("uploading... [inodeNo=%s][dentryname=%s][size=%jd]", inodeNoStr.c_str(),
                                                                         dentryname.c_str(),
