...
// Utilities
//-------------------------------------------------------------------
// [ISSUE2026040800001 M2] make_md5_from_string removed. It used tmpfile()
// to detour the SSE-C key through /tmp before computing MD5, which violated
// CWE-312 / FIPS 140-3 / PCI-DSS / NIST SP 800-57. Replaced by the existing
// in-memory s3fs_get_content_md5_hws_obs at the single call site in
// PushbackSseKeys. See issue for byte-level equivalence proof.
...
{
  // [ISSUE2026040800002] mLock pthread_mutex_init removed (mutex was never
  // lock()/unlock()'d anywhere — pure dead member). Side benefit: removes
  // a latent leak where a successful mutex_init followed by a failed
  // spin_init would return without destroying the already-initialized mutex.
/* file gateway modify begin */
  if (0 != pthread_spin_init(&spinmLock, PTHREAD_PROCESS_PRIVATE)) {
    S3FS_PRN_ERR("Init curl handlers spinmlock failed");
    return false;
  }
/* file gateway modify end */
...
bool CurlHandlerPool::Destroy()
{
  ...
  // [ISSUE2026040800002] mLock pthread_mutex_destroy removed. Side benefit:
  // removes a latent bug where a failed mutex_destroy would return early
  // and skip the spin_destroy below, leaking the spinlock.
/* file gateway modify begin */
  if (0 != pthread_spin_destroy(&spinmLock)) {
    S3FS_PRN_ERR("Destroy curl handlers spinmlock failed");
    return false;
  }
/* file gateway modify end */
  return true;
...
/* file gateway modify begin */
  pthread_spin_lock(&spinmLock);
  if (mIndex >= 0) {
    S3FS_PRN_DBG("Get handler from pool: %d", mIndex);
    h = mHandlers[mIndex--];
  }
  pthread_spin_unlock(&spinmLock);
/* file gateway modify end */
  if (!h) {
...
/* file gateway modify begin */
int              S3fsCurl::sCurlPoolSize       = 200;
/* file gateway modify end */
...
/* file gateway modify begin */
bool             S3fsCurl::is_sigv4            = false;           // default
/* file gateway modify end */
...
/* file gateway modify begin */
  if(0 != pthread_spin_init(&S3fsCurl::curl_handles_spinlock, PTHREAD_PROCESS_PRIVATE)){
    return false;
  }
  if(0 != pthread_rwlock_init(&S3fsCurl::curl_aksk_rwlock, NULL)){
    return false;
  }
/* file gateway modify end */
...
/* file gateway modify begin */
  if(0 != pthread_spin_destroy(&S3fsCurl::curl_handles_spinlock)){
    result = false;
  }
  if(0 !=pthread_rwlock_destroy(&S3fsCurl::curl_aksk_rwlock)){
    result = false;
  }
/* file gateway modify end */
  return result;
...
/* file gateway modify begin */
size_t S3fsCurl::ReadUploadCache(void* ptr, size_t blockSize, size_t numBlocks, void* data)
{
    tagDataBuffer *databuff     = (tagDataBuffer*)data;
    int userBufferRemainSize    = 0;
    unsigned long setBufferSize = 0;
    ...
    if ((off_t)databuff->ulBufferSize < databuff->offset)
    {
        S3FS_PRN_ERR("CurlRest: User buffer has no Data need to update, bufersize:(%zu), readIndex:(%ld), size=(%zu), nmemb=(%zu)",
                databuff->ulBufferSize, (long)databuff->offset, blockSize, numBlocks);
        return 0;
    }
    ...
    if (userBufferRemainSize == 0)
    {
        S3FS_PRN_DBG("CurlRest: User bufersize is %d, copy end, User bufferize:%lu,  RemainSize:%d, curl buffersize:%lu",
            userBufferRemainSize, databuff->ulBufferSize, userBufferRemainSize, setBufferSize);
        return 0;
    }
    else if (userBufferRemainSize < 0)
    {
        S3FS_PRN_WARN("CurlRest: User bufersize is %d, copy end, User bufferize:%lu,  RemainSize:%d, curl buffersize:%lu",
            userBufferRemainSize, databuff->ulBufferSize, userBufferRemainSize, setBufferSize);
        return 0;
    }
    else
    {
        if (setBufferSize > (unsigned long) userBufferRemainSize)
        {
            memcpy(ptr, databuff->pcBuffer + databuff->offset, userBufferRemainSize);
            databuff->offset += userBufferRemainSize;
            S3FS_PRN_DBG("CurlRest: User buffer less, the new user buffer index is %ld.", databuff->offset);

            return userBufferRemainSize;
        }
        else
        {
...
size_t S3fsCurl::ReadCallback(void* ptr, size_t size, size_t nmemb, void* userp)
{
  ...
  if(0 == pCurl->postdata_remaining){  // unsigned, use == 0
    return 0;
  }
...
size_t S3fsCurl::HeaderCallback(void* data, size_t blockSize, size_t numBlocks, void* userPtr)
{
  ...
  if(getline(ss, key, ':')){
    ...
    if(lkey.compare(0, 5, "x-amz") == 0 || lkey.compare(0, 5, "x-hws") == 0){
      key = lkey;
    }
    string value;
    getline(ss, value);
...
size_t S3fsCurl::DownloadWriteCallback(void* ptr, size_t size, size_t nmemb, void* userp)
{
  ...
  // Check HTTP response code before writing to fd.
  // For 4xx/5xx errors, buffer the error body instead of polluting tmpfile.
  long response_code = 0;
  if(CURLE_OK == curl_easy_getinfo(pCurl->hCurl, CURLINFO_RESPONSE_CODE, &response_code)
     && response_code >= 400){
    if(pCurl->bodydata){
      pCurl->bodydata->Append(ptr, size * nmemb);
    }
    return 0;
  }
...
/* file gateway modify begin */
size_t S3fsCurl::DownloadWriteCallbackforRead(void* ptr, size_t size, size_t nmemb, void* userp)
{
  S3fsCurl* pCurl = reinterpret_cast<S3fsCurl*>(userp);
  ...
  // Check HTTP response code before writing to read_buf.
  // For 4xx/5xx errors, buffer the error body instead of polluting the read buffer.
  long response_code = 0;
  if(CURLE_OK == curl_easy_getinfo(pCurl->hCurl, CURLINFO_RESPONSE_CODE, &response_code)
     && response_code >= 400){
    if(pCurl->bodydata){
  ...
  if(NULL == pCurl->read_buf || 0 == pCurl->read_size){
    return 0;
  }

  S3FS_PRN_DBG("[readSize=%zu][dataSize=%zu][cur_pos=%zu]", pCurl->read_size,
    size * nmemb, pCurl->cur_pos);

  size_t remainSize = pCurl->read_size - pCurl->cur_pos;
  // write size
  size_t copysize = (size * nmemb) < remainSize ? (size * nmemb) : remainSize;

  memcpy(pCurl->read_buf + pCurl->cur_pos, &((char*)ptr)[0], copysize);

  if (copysize >= sizeof(long))
  {
    S3FS_PRN_DBG("dataprefix[0x%lx]", *(long*)ptr);
  }

  pCurl->cur_pos += copysize;
...
bool S3fsCurl::PushbackSseKeys(string& onekey)
{
  ...
  // [ISSUE2026040800001 M1] Compute MD5 directly from memory buffer to avoid
  // writing plaintext cryptographic key material to a temporary file.
  // This is byte-equivalent to the previous make_md5_from_string path for all
  // reachable SSE-C key inputs (see issue for the equivalence proof); it also
  // incidentally fixes a latent bug where the old strlen-based MD5 disagreed
  // with the length()-based base64 for keys containing embedded NUL bytes
  // (rejected by OBS server as HTTP 400 InvalidDigest).
  // Same API already used in production at curl.cpp:4153 for Content-MD5
  // header computation, confirming server-side byte compatibility.
  string strMd5 = s3fs_get_content_md5_hws_obs(onekey.c_str(), 0, onekey.length());
  if(strMd5.empty()){
    return false;
  }
  // mapped MD5 = SSE Key
...
bool S3fsCurl::LoadEnvSseCKeys(void)
{
  ...
  if(0 == S3fsCurl::sseckeys.size()){
    // [ISSUE2026040700001 P0-1] Do NOT log envkeys contents — it is the raw
    // SSE-C key material. Only report the input length so operators can
    // distinguish "empty env var" vs "malformed content" without leaking
    // the cryptographic key to syslog/ELK/SIEM (CWE-312, CWE-532).
    S3FS_PRN_ERR("AWSSSECKEYS environment variable does not contain any valid SSE-C key (input length=%zu).",
                 strlen(envkeys));
    return false;
  }
  return true;
...
S3fsCurl* S3fsCurl::CopyMultipartPostRetryCallback(S3fsCurl* s3fscurl)
{
  ...
  // Rebuild copy meta from saved copy-source info
  headers_t retry_meta;
  retry_meta["x-amz-copy-source"] = s3fscurl->b_copy_source;
  retry_meta["x-amz-copy-source-range"] = s3fscurl->b_copy_source_range;

  // setup new curl object
  if(0 != newcurl->CopyMultipartPostSetup(s3fscurl->path.c_str(), s3fscurl->path.c_str(), part_num, upload_id, retry_meta)){
    S3FS_PRN_ERR("Could not duplicate curl object(%s:%d).", s3fscurl->path.c_str(), part_num);
    delete newcurl;
    return NULL;
  }
...
bool S3fsCurl::MixMultipartPostCallback(S3fsCurl* s3fscurl)
{
  ...
  if(REQTYPE_COPYMULTIPOST == s3fscurl->type){
    return s3fscurl->CopyMultipartPostComplete();
  }
  return s3fscurl->UploadMultipartPostComplete();
}
...
S3fsCurl* S3fsCurl::MixMultipartPostRetryCallback(S3fsCurl* s3fscurl)
{
  ...
  // Dispatch to correct retry callback based on request type,
  // mirroring MixMultipartPostCallback (success) pattern.
  if(REQTYPE_COPYMULTIPOST == s3fscurl->type){
    return CopyMultipartPostRetryCallback(s3fscurl);
  }
  return UploadMultipartPostRetryCallback(s3fscurl);
}
...
int S3fsCurl::MixMultipartUploadRequest(const char* tpath, headers_t& meta, int fd,
                                         const std::list<struct fdpage*>& mixuppages)
{
  int            result;
  string         upload_id;
  struct stat    st;
  ...
  // Prepare copy source header
  string copy_source;
  string copy_source_url;
  MakeUrlResource(get_realpath(tpath).c_str(), copy_source, copy_source_url);

  // Build part list from mixuppages
  int part_num = 0;
  S3fsMultiCurl curlmulti;
  ...
  int para_cnt = 0;
  for(std::list<struct fdpage*>::const_iterator iter = mixuppages.begin(); iter != mixuppages.end(); ++iter){
    if((*iter)->modified){
      // UploadPart — upload from local fd
      S3fsCurl* s3fscurl_para            = new S3fsCurl(true);
      s3fscurl_para->partdata.fd         = fd2;
      ...
      part_num++;
      if(0 != (result = s3fscurl_para->UploadMultipartPostSetup(tpath, part_num, upload_id))){
        S3FS_PRN_ERR("failed uploading part setup(%d)", result);
        close(fd2);
        delete s3fscurl_para;
        return result;
      ...
      // CopyPart — server-side copy, split by MAX_MULTI_COPY_SOURCE_SIZE (500MB)
      off_t copy_offset = (*iter)->offset;
      size_t copy_remaining = (*iter)->bytes;
      while(copy_remaining > 0){
        size_t chunk = copy_remaining > static_cast<size_t>(MAX_MULTI_COPY_SOURCE_SIZE)
                       ? static_cast<size_t>(MAX_MULTI_COPY_SOURCE_SIZE) : copy_remaining;

        headers_t copy_meta;
        copy_meta["x-amz-copy-source"] = copy_source;

        // Set copy source range
        off_t range_end = copy_offset + static_cast<off_t>(chunk) - 1;
        char range_buf[128];
        snprintf(range_buf, sizeof(range_buf), "bytes=%jd-%jd", (intmax_t)copy_offset, (intmax_t)range_end);
        copy_meta["x-amz-copy-source-range"] = string(range_buf);

        S3fsCurl* s3fscurl_para = new S3fsCurl(true);
        s3fscurl_para->partdata.add_etag_list(&list);

        part_num++;
        if(0 != (result = s3fscurl_para->CopyMultipartPostSetup(tpath, tpath, part_num, upload_id, copy_meta))){
          S3FS_PRN_ERR("failed copy part setup(%d)", result);
          close(fd2);
          delete s3fscurl_para;
          return result;
        }
    ...
    if(para_cnt >= S3fsCurl::max_parallel_cnt){
      if(0 != (result = curlmulti.Request())){
        S3FS_PRN_ERR("error occurred in multi request(errno=%d).", result);
        close(fd2);
        return result;
...
bool S3fsCurl::DetectBucketTypeFromHeaders(const headers_t& headers)
{
    // Check for OBS file bucket headers returned by HeadBucket (HEAD /)
    // File buckets return: x-obs-bucket-type: POSIX, x-obs-fs-file-interface: Enabled
    // Object buckets do NOT return these headers
    headers_t::const_iterator it;

    it = headers.find("x-obs-bucket-type");
    if (it != headers.end() && strcasecmp(it->second.c_str(), "POSIX") == 0) {
        S3FS_PRN_INFO("Detected FILE_SEMANTIC bucket (x-obs-bucket-type: POSIX)");
        g_bucket_type = BUCKET_TYPE_FILE_SEMANTIC;
        return true;
    }

    it = headers.find("x-obs-fs-file-interface");
    if (it != headers.end() && strcasecmp(it->second.c_str(), "Enabled") == 0) {
        S3FS_PRN_INFO("Detected FILE_SEMANTIC bucket (x-obs-fs-file-interface: Enabled)");
        g_bucket_type = BUCKET_TYPE_FILE_SEMANTIC;
        return true;
    }

    S3FS_PRN_INFO3("No file semantic headers detected");
    return false;
}


// [ISSUE2026040700001 P0-2] Sensitive HTTP header prefixes (case-insensitive)
// whose values must be redacted in verbose logs to avoid leaking credentials
// and encryption keys. Match is exact on "Header-Name:" — partial-name
// headers (e.g. "X-My-Authorization-Header:") are intentionally not matched.
static const char* const SENSITIVE_HEADER_PREFIXES[] = {
    "authorization:",
...
static bool is_sensitive_header_line(const char* line, size_t len)
{
  for(int i = 0; SENSITIVE_HEADER_PREFIXES[i]; i++){
    size_t plen = strlen(SENSITIVE_HEADER_PREFIXES[i]);
    if(len >= plen && 0 == strncasecmp(line, SENSITIVE_HEADER_PREFIXES[i], plen)){
      return true;
    }
  }
  return false;
...
// [ISSUE2026040700001 P0-2] Verbose-output emitter used by CurlDebugFunc.
// Destination by mode:
//   - LOG_MODE_FOREGROUND → stderr (matches libcurl native verbose convention
//     and the Unix standard of writing diagnostic/debug output to stderr).
//     We intentionally do NOT go through S3FS_PRN_CURL here because that
//     macro routes foreground mode to stdout, which would break the
//     standard stream separation for tools like `obsfs -f -d -o curldbg`.
//   - non-foreground (syslog / obsfs log) → forward through S3FS_PRN_CURL
//     so the message picks up the normal log-framework formatting and
//     destination (syslog / IndexLog).
// Buffer size 4KB covers typical HTTP header lines including the 4096-'A'
// stress-test case; anything longer is truncated (best-effort).
static void curl_verbose_emit(const char* fmt, ...) __attribute__((format(printf, 1, 2)));
static void curl_verbose_emit(const char* fmt, ...)
{
  va_list ap;
  va_start(ap, fmt);
  if(LOG_MODE_FOREGROUND == debug_log_mode){
    ...
    // Render once into a fixed buffer, then forward through the standard
    // log macro so syslog / IndexLog see the normal framing.
    char buf[4096];
...
int S3fsCurl::CurlDebugFunc(CURL* hcurl, curl_infotype type, char* data, size_t size, void* userptr)
{
  ...
  switch(type){
    case CURLINFO_TEXT:
      ...
      curl_verbose_emit("* %*s%.*s", indent, "", (int)size, data);
      break;
    ...
    case CURLINFO_HEADER_OUT:
      ...
      do {
        ...
        // [ISSUE2026040700001 P2-1] When neither '\n' nor '\r' is found,
        // treat the remaining buffer as the final line. The old code did
        // `length = eol - p` with eol==NULL which is undefined behavior
        // (NULL pointer arithmetic) and — combined with the new
        // is_sensitive_header_line(p, line_len) call — could cause OOB
        // reads when the input is shorter than the longest sensitive
        // prefix. libcurl's HEADER_IN/OUT buffers are not guaranteed to
        // be NUL-terminated, so the pre-fix "accidentally works" behavior
        // on test inputs was not safe in general.
        length = (eol != NULL) ? static_cast<size_t>(eol - p) : remaining;
        // [ISSUE2026040700001 P0-2] Redact sensitive header values (Authorization,
        // security-token, SSE-C key, etc.) before writing to verbose log.
        size_t line_len = length - newline;
        if(is_sensitive_header_line(p, line_len)){
          const char* colon = (const char*)memchr(p, ':', line_len);
          int prefix_len = (colon ? (int)(colon - p + 1) : (int)line_len);
          curl_verbose_emit("%c %.*s ***REDACTED***",
                            CURLINFO_HEADER_IN == type ? '<' : '>',
                            prefix_len, p);
        } else {
          curl_verbose_emit("%c %.*s",
                            CURLINFO_HEADER_IN == type ? '<' : '>',
                            (int)line_len, p);
        }
        remaining -= length;
...
S3fsCurl::S3fsCurl(bool ahbe) :
    ...
    /* [ISSUE2026040700002 task G] migrated 7 trivial member init from body to init list */
    writedatabuf{}, read_buf(NULL), read_startpos(0), read_size(0), cur_pos(0)
{
}
...
bool S3fsCurl::ResetHandle(void)
{
  ...
  if(S3fsCurl::is_verbose){
    ...
    // [ISSUE2026040700001 P0-2] Always register CurlDebugFunc so the sensitive
    // header redaction applies in foreground mode too. Without this, libcurl
    // writes verbose output directly to stderr, bypassing the redaction and
    // leaking Authorization / x-amz-security-token / SSE-C key headers.
...
{
  S3FS_PRN_INFO3("Retry request. [type=%d][url=%s]", type, url.c_str());
  ...
  switch(type){
    ...
    case REQTYPE_PUT:
      ...
      if(g_bucket_type == BUCKET_TYPE_OBJECT_SEMANTIC){
        // Object bucket: restore fd-based upload from PutRequest
        if(b_infile){
...
/* file gateway modify begin */
        // File bucket: original buffer-based upload logic
        if(NULL != writedatabuf.pcBuffer){
          writedatabuf.offset = 0;
          curl_easy_setopt(hCurl, CURLOPT_READDATA, &writedatabuf);
          curl_easy_setopt(hCurl, CURLOPT_READFUNCTION, ReadUploadCache);
          curl_easy_setopt(hCurl, CURLOPT_INFILESIZE, writedatabuf.ulBufferSize);
          S3FS_PRN_INFO("[REQTYPE_PUT] Setting upload: size=%lu",
              writedatabuf.ulBufferSize);  // removed raw pointer address from log
        }else{
          S3FS_PRN_WARN("[REQTYPE_PUT] writedatabuf.pcBuffer is NULL, setting INFILESIZE to 0!");
          curl_easy_setopt(hCurl, CURLOPT_INFILESIZE, 0);
        }
/* file gateway modify end */
      }
    ...
    case REQTYPE_GET:
      ...
      if(g_bucket_type == BUCKET_TYPE_OBJECT_SEMANTIC){
        // Object bucket: restore fd-based download from GetObjectRequest
...
/* file gateway modify begin */
        // File bucket: original buffer-based download logic
        curl_easy_setopt(hCurl, CURLOPT_WRITEFUNCTION, S3fsCurl::DownloadWriteCallbackforRead);
        curl_easy_setopt(hCurl, CURLOPT_WRITEDATA, (void*)this);
        // responseHeaders
        curl_easy_setopt(hCurl, CURLOPT_HEADERDATA, (void*)&responseHeaders);
        curl_easy_setopt(hCurl, CURLOPT_HEADERFUNCTION, HeaderCallback);
        if(NULL != read_buf){
          if(cur_pos > 0){
            S3FS_PRN_WARN("### GET err, cur_pos: %zu, url: %s", cur_pos, url.c_str());
          }
...
/* add for object_file_gateway*/
std::string S3fsCurl::PrintRequestId4ResponseErr()
{
  std::string requestIdStr = "";
  bool logflag = true;
  do
  {
    if ( LastResponseCode < 400 && LastResponseCode > 0)
    {
      logflag = false;
      break;
    }
    headers_t::iterator it = responseHeaders.find("x-amz-request-id");
    if (it == responseHeaders.end())
    {
      break;
    }

    requestIdStr = it->second;
  } while(0);

  if (404 == LastResponseCode || 416 == LastResponseCode)
  {
    S3FS_PRN_INFO("curl perform failed,url=%s,response=%ld,requestid=%s",
        url.c_str(),LastResponseCode,requestIdStr.c_str());
  }
  else if (logflag)
  {
    S3FS_PRN_ERR("curl perform failed,url=%s,response=%ld,requestid=%s",
        url.c_str(),LastResponseCode,requestIdStr.c_str());
  }
...
CURLcode S3fsCurl::Hws_curl_easy_perform()
{
...
/* file gateway modify begin */
    ...
    S3FS_PRN_DBG("RequestPerform: about to call Hws_curl_easy_perform...");
    CURLcode curlCode = Hws_curl_easy_perform();
    S3FS_PRN_DBG("RequestPerform: Hws_curl_easy_perform returned, curlCode=%d", curlCode);
    s3fsStatisEnd(LIB_CUR_SEND, &begin_tv);
    S3FS_PRN_DBG("RequestPerform: s3fsStatisEnd completed");
...
/* file gateway modify end */
    ...
    switch(curlCode){
      case CURLE_OK:
      {
        ...
        std::string requestIdStr = PrintRequestId4ResponseErr();

        if(0 == LastResponseCode){
        ...
        switch(LastResponseCode){
          case 400:
            S3FS_PRN_INFO3("HTTP response code 400 was returned, returning EIO. requestId: %s", (requestIdStr.c_str()));
            S3FS_PRN_WARN("Body Text: %s, requestId: %s", (bodydata ? bodydata->str() : ""), (requestIdStr.c_str()));
            return -EIO;

          case 403:
            S3FS_PRN_INFO3("HTTP response code 403 was returned, returning EPERM. requestId: %s", (requestIdStr.c_str()));
            S3FS_PRN_WARN("Body Text: %s, requestId: %s", (bodydata ? bodydata->str() : ""), (requestIdStr.c_str()));
            return -EPERM;

          case 404:
            S3FS_PRN_INFO3("HTTP response code 404 was returned, returning ENOENT. requestId: %s", (requestIdStr.c_str()));
            S3FS_PRN_DBG("Body Text: %s, requestId: %s", (bodydata ? bodydata->str() : ""), (requestIdStr.c_str()));
            return -ENOENT;

		  case 405:
            S3FS_PRN_INFO3("HTTP response code 405 was returned, this bucket not suport fs, cannt mount. requestId: %s", (requestIdStr.c_str()));
            S3FS_PRN_WARN("Body Text: %s, requestId: %s", (bodydata ? bodydata->str() : ""), (requestIdStr.c_str()));
            return -EIO;

          default:
            S3FS_PRN_INFO3("HTTP response code = %ld, returning EIO. requestId: %s", LastResponseCode, (requestIdStr.c_str()));
            S3FS_PRN_DBG("Body Text: %s, requestId: %s", (bodydata ? bodydata->str() : ""), (requestIdStr.c_str()));
            return -EIO;
          }
		}
      ...
      case CURLE_WRITE_ERROR:
        curl_easy_getinfo(hCurl, CURLINFO_RESPONSE_CODE, &LastResponseCode);

        if(400 <= LastResponseCode && LastResponseCode < 500){
            S3FS_PRN_ERR("HTTP %ld on GET download: %s", LastResponseCode, url.c_str());
            if(bodydata){
                S3FS_PRN_WARN("Body Text: %s", bodydata->str());
            }
            if(403 == LastResponseCode){
                if(bodydata && bodydata->size() > 0 && strstr(bodydata->str(), "InvalidObjectState")){
                    S3FS_PRN_ERR("Object is in GLACIER/archive storage, cannot read directly");
                    return -EREMOTE;
                }
                return -EPERM;
            }
            if(404 == LastResponseCode){ return -ENOENT; }
            if(416 == LastResponseCode){ return -ERANGE; }
            return -EIO;

        }else if(500 <= LastResponseCode){
            S3FS_PRN_ERR("HTTP %ld (server error) on GET download: %s", LastResponseCode, url.c_str());
            if(bodydata){
                S3FS_PRN_WARN("Body Text: %s", bodydata->str());
            }
            sleep(4);
      ...
      case CURLE_OPERATION_TIMEDOUT:
        S3FS_PRN_ERR("### CURLE_OPERATION_TIMEDOUT, url: %s", url.c_str());
        sleep(2);
        break;
	...
	sleepSeconds = (retrycnt + 1) * 3 * sleepSecondStep;
      ...
      case CURLE_GOT_NOTHING:
        S3FS_PRN_ERR("### CURLE_GOT_NOTHING, url: %s", url.c_str());
        sleep(4);
        break;

      case CURLE_ABORTED_BY_CALLBACK:
        S3FS_PRN_ERR("### CURLE_ABORTED_BY_CALLBACK,op=%s,wrt_off=%u,rd_off=%u,url=%s",
                op.c_str(),(unsigned int)writedatabuf.offset,
                (unsigned int)read_startpos,url.c_str());
        sleep(4);
        S3fsCurl::curl_times[hCurl] = time(0);
        break;

      case CURLE_PARTIAL_FILE:
        S3FS_PRN_ERR("### CURLE_PARTIAL_FILE, url: %s", url.c_str());
        sleep(4);
        break;

      case CURLE_SEND_ERROR:
        S3FS_PRN_ERR("### CURLE_SEND_ERROR, url: %s", url.c_str());
        sleep(2);
        break;

      case CURLE_RECV_ERROR:
        S3FS_PRN_ERR("### CURLE_RECV_ERROR, url: %s", url.c_str());
        sleep(2);
        break;

      case CURLE_SSL_CONNECT_ERROR:
        S3FS_PRN_ERR("### CURLE_SSL_CONNECT_ERROR, url: %s", url.c_str());
        sleep(2);
        break;

      case CURLE_SSL_CACERT:
        S3FS_PRN_ERR("### CURLE_SSL_CACERT, url: %s", url.c_str());

        // try to locate cert, if successful, then set the
        // option and continue
        if(0 == S3fsCurl::curl_ca_bundle.size()){
          if(!S3fsCurl::LocateBundle()){
            S3FS_PRN_ERR("could not get CURL_CA_BUNDLE, url: %s", url.c_str());
            return -EIO;
          }
      ...
      case CURLE_PEER_FAILED_VERIFICATION:
        S3FS_PRN_ERR("### CURLE_PEER_FAILED_VERIFICATION, url: %s", url.c_str());
      ...
      case CURLE_HTTP_RETURNED_ERROR:
        S3FS_PRN_ERR("### CURLE_HTTP_RETURNED_ERROR, url: %s", url.c_str());

        if(0 != curl_easy_getinfo(hCurl, CURLINFO_RESPONSE_CODE, &LastResponseCode)){
          return -EIO;
        }
        S3FS_PRN_INFO3("HTTP response code =%ld, url: %s", LastResponseCode, url.c_str());
    ...
    if(!RemakeHandle()){
/* file gateway modify begin */
      S3FS_PRN_ERR("Failed to reset handle and internal data for retrying, url: %s", url.c_str());
/* file gateway modify end */
      return -EIO;
    }
  }
  S3FS_PRN_ERR("### giving up, url: %s", url.c_str());
...
string S3fsCurl::CalcSignatureV2(const string& method, const string& strMD5, const string& content_type, const string& date, const string& resource, const std::string& sk, const std::string& token)
{
  ...
  if(!s3fs_HMAC(key, key_len, sdata, sdata_len, &md, &md_len)){  // check HMAC return value
    return string("");
  }
...
string S3fsCurl::CalcSignature(const string& method, const string& canonical_uri, const string& query, const string& strdate, const string& payload_hash, const string& date8601, const std::string& sk, const std::string& token)
{
  ...
  // Chain HMAC256 for signing key derivation; check each return value
  if(!s3fs_HMAC256(kSecret, kSecret_len, reinterpret_cast<const unsigned char*>(strdate.data()), strdate.size(), &kDate, &kDate_len)){
    return "";  // kDate allocation failed, nothing to free
  }
  if(!s3fs_HMAC256(kDate, kDate_len, reinterpret_cast<const unsigned char*>(endpoint.c_str()), endpoint.size(), &kRegion, &kRegion_len)){
    free(kDate);
    return "";
  }
  if(!s3fs_HMAC256(kRegion, kRegion_len, reinterpret_cast<const unsigned char*>("s3"), sizeof("s3") - 1, &kService, &kService_len)){
    free(kDate);
    free(kRegion);
    return "";
  }
  if(!s3fs_HMAC256(kService, kService_len, reinterpret_cast<const unsigned char*>("aws4_request"), sizeof("aws4_request") - 1, &kSigning, &kSigning_len)){
    free(kDate);
    free(kRegion);
    free(kService);
    return "";
  ...
  if(!s3fs_sha256(cRequest, cRequest_len, &sRequest, &sRequest_len)){  // check sha256 return value
    free(kSigning);
    return "";
  }
  ...
  if(!s3fs_HMAC256(kSigning, kSigning_len, cscope, cscope_len, &md, &md_len)){  // check final HMAC256
    free(kSigning);
    return "";
  }
...
void S3fsCurl::insertV4Headers(const std::string& ak, const std::string& sk, const std::string& token)
{
  ...
  switch (type) {
    case REQTYPE_PUT:
/* file gateway modify begin */
      /* add for object_file_gateway , computer bufs signature*/
      payload_hash = s3fs_sha256sum_hw_obs(writedatabuf.pcBuffer, writedatabuf.offset, writedatabuf.ulBufferSize);
      // Do NOT log buf as %s — pcBuffer holds raw upload bytes (not NUL-terminated),
      // would cause OOB read and leak user data into DBG log.
      S3FS_PRN_DBG("[payload_hash=%s]", payload_hash.c_str());
/* file gateway modify end */
      break;

    case REQTYPE_COMPLETEMULTIPOST:
    {
      if(!b_postdata){ break; }  // NULL defense: avoid crash on strlen(NULL)
      ...
      char                 hexsRequest[64 + 1];
      unsigned int         cnt;
      if(!s3fs_sha256(b_postdata, cRequest_len, &sRequest, &sRequest_len)){  // check sha256 return
        break;  // payload_hash remains empty -> empty_payload_hash used -> OBS 403
      }
      for(cnt = 0; cnt < sRequest_len; cnt++){
        sprintf(&hexsRequest[cnt * 2], "%02x", sRequest[cnt]);
      }
...
/* file gateway modify begin */
  // Only add OBS-specific header for file semantic buckets
  if(g_bucket_type == BUCKET_TYPE_FILE_SEMANTIC){
...
/* file gateway modify end */
  ...
  op = "DELETE";
  type = REQTYPE_DELETE;
  insertAuthHeaders(S3fsCurl::AWSAccessKeyId, S3fsCurl::AWSSecretAccessKey, S3fsCurl::AWSAccessToken);
...
/* file gateway modify begin */
    // Only add OBS-specific headers for file semantic buckets
    if(g_bucket_type == BUCKET_TYPE_FILE_SEMANTIC){
...
int S3fsCurl::HeadRequest(const char* tpath, headers_t& meta, long long* inode_no, const char* shardkey)
{
  ...
  /*if(!PreHeadRequest(tpath, NULL, NULL, -1, inode_no, shardkey) || 0 != (result = RequestPerform())){
    // If has SSE-C keys, try to get with all SSE-C keys.
    for(int pos = 0; static_cast<size_t>(pos) < S3fsCurl::sseckeys.size(); pos++){
      if(!DestroyCurlHandle()){
        return result;
      }
      if(!PreHeadRequest(tpath, NULL, NULL, pos, inode_no, shardkey)){
        return result;
      }
  ...
  bool Preflag = (int) PreHeadRequest(tpath, NULL, NULL, -1, inode_no, shardkey);
  if (!Preflag){
    S3FS_PRN_ERR("PreHeadRequest failed, inde_no = %lld, shardkey = %s",
        (inode_no ? *inode_no : -1), SAFESTRPTR(shardkey));  // NULL defense: avoid crash when optional params are NULL
    return -1;
  }

  timeval  begin_tv;
  s3fsStatisStart(GETATTR_LIB_CURL, &begin_tv);
  result = RequestPerform();
  s3fsStatisEnd(GETATTR_LIB_CURL, &begin_tv,url.c_str());
  if (0 != result){
    S3FS_PRN_INFO("result=%d", result);
  ...
  for(headers_t::iterator iter = responseHeaders.begin(); iter != responseHeaders.end(); ++iter){
    ...
    if(key == "content-type"){
      ...
      meta[iter->first] = value;
    }else if(key == hws_s3fs_shardkey){
      meta[iter->first] = value;
    }else if(key == hws_s3fs_inodeNo){
      meta[iter->first] = value;
    }else if(key.substr(0, 5) == "x-amz"){
      meta[key] = value;		// key is lower case for "x-amz"
    }else if(key.substr(0, 5) == "x-hws"){
      meta[key] = value;		// key is lower case for "x-hws"
    }
  }
  return 0;
}
...
int S3fsCurl::PutHeadRequest(const char* tpath, headers_t& meta, bool is_copy,
                                        const string& out_query_string)
{
  S3FS_PRN_INFO2("[tpath=%s]", SAFESTRPTR(tpath));
  ...
  if (!out_query_string.empty())
  {
      string urlargs = "?" + out_query_string;
      turl           += urlargs;
      query_string   = out_query_string;
  }
  url             = prepare_url(turl.c_str());
  path            = get_realpath(tpath);
  requestHeaders  = NULL;
...
/* file gateway modify begin */
  // Only add OBS-specific header for file semantic buckets
  // For object semantic buckets (S3), this header causes errors
  if(g_bucket_type == BUCKET_TYPE_FILE_SEMANTIC){
    requestHeaders = curl_slist_sort_insert(requestHeaders, hws_s3fs_client, "true");
  }
/* file gateway modify end */
  ...
  S3FS_PRN_INFO2("copying... [path=%s]", tpath);
...
int S3fsCurl::PutRequest(const char* tpath, headers_t& meta, int fd)
{
  ...
  // Debug: Log response body when result is 0 to check for hidden errors
  if(result == 0 && bodydata) {
    const char* pstrbody = bodydata->str();
    size_t bodylen = bodydata->size();
    S3FS_PRN_INFO("[PutRequest] HTTP result=0, body length=%zu", bodylen);
    if(bodylen > 0 && bodylen < 1000) {
      S3FS_PRN_INFO("[PutRequest] Response body: %s", pstrbody);
    }
    // Check for error indicators in response body
    std::string bodystr(pstrbody, bodylen);
    if(bodystr.find("<Error>") != std::string::npos ||
       bodystr.find("<Code>") != std::string::npos) {
      S3FS_PRN_ERR("[PutRequest] HTTP 200 but body contains error: %s", pstrbody);
      // Don't fail yet, just log for debugging
    }
  }
...
/* file gateway modify begin */
int S3fsCurl::CreateZeroByteDirObject(const char* tpath, headers_t& meta)
{
  S3FS_PRN_INFO3("[tpath=%s]", SAFESTRPTR(tpath));
...
/* file gateway modify end */
  ...
  // add responseHeaders for shardkey and inodeno
  curl_easy_setopt(hCurl, CURLOPT_HEADERDATA, (void*)&responseHeaders);
  curl_easy_setopt(hCurl, CURLOPT_HEADERFUNCTION, HeaderCallback);

  S3fsCurl::AddUserAgent(hCurl);                                // put User-Agent

  S3FS_PRN_INFO3("uploading... [path=%s]", tpath);
...
int S3fsCurl::CreateZeroByteFileObject(const char* tpath, headers_t& meta)
{
  S3FS_PRN_INFO3("[tpath=%s]", SAFESTRPTR(tpath));
  ...
  s3fsStatisStart(CREATE_CUR_HANDLE_DELAY, &create_handle_tv);
  if(!CreateCurlHandle(true)){
    return -1;
  }
  s3fsStatisEnd(CREATE_CUR_HANDLE_DELAY, &create_handle_tv);

  string resource;
  string turl;
  timeval create_makeurl_tv;
  s3fsStatisStart(CREATE_MAKE_CURL_DELAY, &create_makeurl_tv);
  MakeUrlResource(get_realpath(tpath).c_str(), resource, turl);

  url             = prepare_url(turl.c_str());
  path            = get_realpath(tpath);
  S3FS_PRN_DBG("[turl=%s],[url=%s],[path=%s]", turl.c_str(), url.c_str(), path.c_str());
  requestHeaders  = NULL;
  responseHeaders.clear();
  bodydata        = new BodyData();
...
/* file gateway modify end */
  ...
  s3fsStatisEnd(CREATE_MAKE_CURL_DELAY, &create_makeurl_tv);

  op = "PUT";
  type = REQTYPE_PUT;
  timeval insertAuth_tv;
  s3fsStatisStart(CREATE_AUTH_DELAY, &insertAuth_tv);
  insertAuthHeaders(S3fsCurl::AWSAccessKeyId, S3fsCurl::AWSSecretAccessKey, S3fsCurl::AWSAccessToken);
  s3fsStatisEnd(CREATE_AUTH_DELAY, &insertAuth_tv);
  // setopt
  curl_easy_setopt(hCurl, CURLOPT_URL, url.c_str());
  curl_easy_setopt(hCurl, CURLOPT_UPLOAD, true);                // HTTP PUT
  ...
  timeval  begin_tv;
  s3fsStatisStart(CREATE_FILE_LIBCURL, &begin_tv);
  int result = RequestPerform();
  s3fsStatisEnd(CREATE_FILE_LIBCURL, &begin_tv);
  delete bodydata;
  bodydata = NULL;
...
int S3fsCurl::PreReadFromFileObject(const char* tpath,
                                              char* buf,
                                              off_t start,
                                              size_t size,
                                              sse_type_t sseType,
                                              ...
                                              const char *inodeNoStr)
{
  ...
  S3FS_PRN_INFO3("[tpath=%s][start=%jd][size=%zu][ssetype=%d][ssevalue=%s]",
  ...
  // Initialize bodydata to capture error response body for 4xx/5xx in DownloadWriteCallbackforRead
  if(bodydata){
    delete bodydata;
  }
  bodydata = new BodyData();
...
int S3fsCurl::ReadFromFileObject(const char* tpath,
                                        char* buf,
                                        off_t start,
                                        size_t size,
                                        const char *shardkey,
                                        const char *inodeNoStr)
{
  ...
  S3FS_PRN_INFO3("[tpath=%s][start=%jd][size=%zu]", SAFESTRPTR(tpath), (intmax_t)start, size);
  ...
  S3FS_PRN_INFO3("reading... [path=%s]", tpath);

  timeval begin_tv;
  s3fsStatisStart(READ_FILE_LIBCURL, &begin_tv);
  result = RequestPerform();
  s3fsStatisEnd(READ_FILE_LIBCURL, &begin_tv);
...
int S3fsCurl::WriteBytesToFileObject(const char    *tpath,
                                     tagDataBuffer *databuff,
                                     off_t          offset,
                                     tag_index_cache_entry_t *fileMeta)
{
    string offsetStr   = toString(offset);
    string inodeNoStr  = toString(fileMeta->inodeNo);
    string dentryname  = fileMeta->dentryname;
    ...
    if(!CreateCurlHandle(true)){
        S3FS_PRN_ERR("CreateCurlHand failed");
        return -1;
    }

    // For object buckets, use standard S3 protocol (like s3fs) instead of OBS-specific modify parameter
    // This ensures compatibility with standard S3 services like s3proxy
    // File buckets retain OBS-specific modify functionality
    if(g_bucket_type == BUCKET_TYPE_OBJECT_SEMANTIC){
        query_string = "";  // Don't use OBS-specific "modify=1&position=" parameter
    }else{
        query_string = writeparmname + offsetStr;
    }
    string urlargs      = query_string.empty() ? "" : "?" + query_string;
    ...
    writedatabuf.pcBuffer     = databuff->pcBuffer;
    writedatabuf.ulBufferSize = databuff->ulBufferSize;
    writedatabuf.offset       = 0;
    // "x-amz-acl", storage class, sse
    if(!S3fsCurl::default_acl.empty()){
        requestHeaders = curl_slist_sort_insert(requestHeaders, "x-amz-acl", S3fsCurl::default_acl.c_str());
    ...
    string strMD5;
    if( NULL != writedatabuf.pcBuffer && S3fsCurl::is_content_md5){
        strMD5         = s3fs_get_content_md5_hws_obs(writedatabuf.pcBuffer, 0, writedatabuf.ulBufferSize);
        requestHeaders = curl_slist_sort_insert(requestHeaders, "Content-MD5", strMD5.c_str());
    }
    ...
    {
        requestHeaders = curl_slist_sort_insert(requestHeaders, hws_s3fs_first_write, "true");
    }
    op = "PUT";
    type = REQTYPE_PUT;
    insertAuthHeaders(S3fsCurl::AWSAccessKeyId, S3fsCurl::AWSSecretAccessKey, S3fsCurl::AWSAccessToken);

    S3FS_PRN_INFO("[WriteBytesToFileObject] Before RequestPerform: type=%d, size=%lu",
        type, writedatabuf.ulBufferSize);  // removed raw pointer address from log
    ...
    S3FS_PRN_INFO3("uploading... [inodeNo=%s][dentryname=%s][size=%zu]", inodeNoStr.c_str(),
                                                                         dentryname.c_str(),
                                                                         writedatabuf.ulBufferSize);
    s3fsStatisStart(WRITE_FILE_LIBCURL, &begin_tv);
    int result = RequestPerform();
    s3fsStatisEnd(WRITE_FILE_LIBCURL, &begin_tv,url.c_str());
    delete bodydata;
    bodydata = NULL;
    if (0 != result){
        S3FS_PRN_ERR("[result=%d][path=%s],s3fs_write_file failed,", result, tpath);
    }
...
int S3fsCurl::RenameFileOrDirObject(const char *from, const char *to, bool& need_check_to)
{
    if(!CreateCurlHandle(true)){
        S3FS_PRN_ERR("CreateCurlHand failed");
        return -1;
    ...
    delete bodydata;
    bodydata = NULL;
    if (0 != result && (0 != retry_count || 404 == LastResponseCode)){
        S3FS_PRN_ERR("[result=%d][from=%s][to=%s][retry_count=%d][LastResponseCode=%ld],s3fs_rename failed,need check to whether exist.", result, from, to, retry_count, LastResponseCode);
        need_check_to = true;
    }
...
int S3fsCurl::PreGetObjectRequest(const char* tpath, int fd, off_t start, ssize_t size, sse_type_t sseType, string& ssevalue)
{
  ...
  // Initialize bodydata to capture error response body for 4xx/5xx in DownloadWriteCallback
  if(bodydata){
    delete bodydata;
  }
  bodydata = new BodyData();
...
int S3fsCurl::getRootInodeNo()
{
    headers_t::iterator it = responseHeaders.find(hws_s3fs_inodeNo);
    if (it == responseHeaders.end())
    {
        S3FS_PRN_ERR("get RootInodeNo Error");
        return -1;
    }
    string value = it->second;
    gRootInodNo = get_inodeNo(value.c_str());
    S3FS_PRN_DBG("get RootInodeNo %lld", gRootInodNo);
    return 0;
}

int S3fsCurl::getMountPrefixInode(void)
{
  S3FS_PRN_INFO3("get PreMount InodeNo");

  headers_t tmpHead;
  path = "/";
  int result  = HeadRequest(path.c_str(), tmpHead, NULL, NULL);
  if (result != 0) {
    S3FS_PRN_ERR("Head PreMount %s failed, result %d", mount_prefix.c_str(), result);
  }

  result = getRootInodeNo();
  if (result != 0) {
    S3FS_PRN_ERR("get RootInodeNo failed, result %d", result);
  }
  return result;
}

//
// Object bucket mount-prefix existence check.
//
// Issues GET /?prefix=<norm>&delimiter=/&max-keys=2&encoding-type=url where
// <norm> = mount_prefix with leading '/' stripped and trailing '/' added.
// Existence is asserted iff the response has any non-empty <Contents><Key>
// or <CommonPrefixes><Prefix> — the directory marker "<prefix>/" counts.
//
// Used in place of getMountPrefixInode() for object buckets, which lack the
// file-bucket-only x-hws-fs-inodeNo response header.  ListBucketRequest's
// existing "add x-hws-fs-client only on FILE_SEMANTIC" guard produces a
// clean S3-style request here (callsite is gated on OBJECT_SEMANTIC).
//
// Returns 0 on existence, -ENOENT not found, -EIO on parse failure.
//
int S3fsCurl::CheckObjectBucketPrefixExists(const std::string& prefix)
{
  S3FS_PRN_INFO3("[prefix=%s]", prefix.c_str());
  ...
  std::string query = "prefix=" + urlEncode(norm) + "&delimiter=/&max-keys=2&encoding-type=url";

  int rc = ListBucketRequest("/", query.c_str());
  if(rc != 0){
    S3FS_PRN_ERR("CheckObjectBucketPrefixExists: ListObjects failed, prefix=%s, http=%ld, rc=%d",
                 norm.c_str(), LastResponseCode, rc);
    DestroyCurlHandle();
    return rc;
  }
  ...
  if(body && body->size() > 0){
    ...
    if(!doc){
      ...
      for(xmlNodePtr el = root ? root->children : NULL; el && !found; el = el->next){
        if(el->type != XML_ELEMENT_NODE){ continue; }
        const char* tag = (const char*)el->name;
        const char* inner_target = NULL;
        if(0 == strcmp(tag, "Contents")){
          inner_target = "Key";
        } else if(0 == strcmp(tag, "CommonPrefixes")){
          inner_target = "Prefix";
        } else {
          continue;
        }
        for(xmlNodePtr inner = el->children; inner && !found; inner = inner->next){
          if(inner->type != XML_ELEMENT_NODE){ continue; }
          if(0 != strcmp((const char*)inner->name, inner_target)){ continue; }
          xmlChar* val = xmlNodeGetContent(inner);
          if(val){
            if(val[0] != '\0'){ found = true; }
            xmlFree(val);
          }
        }
      }
  ...
  if(parse_failed){
    S3FS_PRN_ERR("CheckObjectBucketPrefixExists: XML parse failed for prefix=%s", norm.c_str());
    return -EIO;
  }
  if(found){
    S3FS_PRN_INFO("CheckObjectBucketPrefixExists: prefix=%s found", norm.c_str());
    return 0;
  }
  // Not-found is a normal return code (used by getattr ENOENT fallback path),
  // not a fault. Log at INFO so stat() on non-existent paths doesn't spam ERR.
  // Callers that treat -ENOENT as fatal (e.g. mount_prefix validation in
  // s3fs_check_service) log their own context-specific PRN_CRIT at the call site.
  S3FS_PRN_INFO("CheckObjectBucketPrefixExists: prefix=%s not found", norm.c_str());
  return -ENOENT;
}
...
int S3fsCurl::CheckBucket(const std::string& ak, const std::string& sk, const std::string& token)
{
  ...
  // Step 1: Auto-detect bucket type via HEAD / (HeadBucket) when not specified
  // OBS HeadBucket returns x-obs-bucket-type: POSIX for file buckets.
  // GET / (ListObjects) does NOT return these headers, so HEAD is required.
  if (g_bucket_type == BUCKET_TYPE_UNKNOWN) {
    S3FS_PRN_INFO("Bucket type not specified, detecting via HEAD /...");
    ...
    int head_result = RequestPerform();

    if (head_result == 0) {
      bool detected = S3fsCurl::DetectBucketTypeFromHeaders(responseHeaders);
      if (!detected) {
        S3FS_PRN_INFO("HEAD / succeeded but no file-semantic headers, auto-detected as OBJECT_SEMANTIC");
        g_bucket_type = BUCKET_TYPE_OBJECT_SEMANTIC;
        nohwscache = true;
      }
    } else {
      S3FS_PRN_WARN("HEAD / for type detection failed (HTTP %ld), skipping auto-detection", LastResponseCode);
    }

    if (g_bucket_type != BUCKET_TYPE_UNKNOWN) {
      const char* type_str = (g_bucket_type == BUCKET_TYPE_FILE_SEMANTIC) ? "FILE_SEMANTIC" : "OBJECT_SEMANTIC";
      S3FS_PRN_INFO("Auto-detected bucket type: %s", type_str);
    }

    DestroyCurlHandle();
  }

  // Step 2: Bucket existence check via GET / (origin/master behavior)
  if(!CreateCurlHandle(true)){
    return -1;
  }
  string resource;
...
/* file gateway modify end */
  ...
  if (result != 0) {
    if (LastResponseCode == 405 &&
        (g_bucket_type == BUCKET_TYPE_OBJECT_SEMANTIC || g_bucket_type == BUCKET_TYPE_UNKNOWN)) {
      S3FS_PRN_WARN("CheckBucket: HTTP 405 is expected for object bucket, treating as success.");
      result = 0;
    } else {
      S3FS_PRN_ERR("CheckBucket failed: HTTP %ld, url=%s", LastResponseCode, url.c_str());
      S3FS_PRN_ERR("CheckBucket response body: %s", (bodydata ? bodydata->str() : "(empty)"));
      char *message = NULL;
    ...
    S3FS_PRN_WARN("CheckBucket: OK (HTTP %ld)", LastResponseCode);
  }
...
int S3fsCurl::UploadMultipartPostSetup(const char* tpath, int part_num, const string& upload_id)
{
  ...
  // Reset request state BEFORE inserting headers. The Content-MD5 insert below
  // would otherwise be orphaned by the `requestHeaders = NULL` line further
  // down — leaking the slist node + base64 string and silently dropping the
  // header from the actual request.
  if(requestHeaders){
    curl_slist_free_all(requestHeaders);
    requestHeaders = NULL;
  }
...
int S3fsCurl::CopyMultipartPostSetup(const char* from, const char* to, int part_num, const string& upload_id, headers_t& meta)
{
  ...
  // Make request headers and save copy source info for retry
  for(headers_t::iterator iter = meta.begin(); iter != meta.end(); ++iter){
    string key   = lower(iter->first);
    string value = iter->second;
    if(key == "content-type"){
...
bool S3fsCurl::CopyMultipartPostComplete()
{
  ...
  if(!bodydata || 0 == bodydata->size()){
    S3FS_PRN_ERR("CopyMultipartPost response body is empty");
    return false;
  }

  xmlDocPtr doc;
  if(NULL == (doc = xmlReadMemory(bodydata->str(), bodydata->size(), "", NULL, 0))){
    S3FS_PRN_ERR("Could not parse CopyMultipartPost response XML");
    return false;
  }
  if(NULL == doc->children){
    S3FS_XMLFREEDOC(doc);
  ...
  for(xmlNodePtr cur_node = doc->children->children; NULL != cur_node; cur_node = cur_node->next){
    if(XML_ELEMENT_NODE == cur_node->type){
      string elementName = reinterpret_cast<const char*>(cur_node->name);
      if(cur_node->children && XML_TEXT_NODE == cur_node->children->type){
        if(elementName == "ETag"){
          string etag = reinterpret_cast<const char*>(cur_node->children->content);
          if(etag.size() >= 2 && *etag.begin() == '"' && *etag.rbegin() == '"'){
            etag.assign(etag.substr(1, etag.size() - 2));
