...
Node* IndexCache::getFreeNodeAndEraseMap(const char* pathStr)
{
    ...
    Node* node = snas_container_of(taillistnode, Node, list_ptr);
...
{
    // For S3 object semantic mode, fill in missing OBS-specific fields
    // This allows IndexCache to work with virtual inodes for S3 buckets
    if (g_bucket_type == BUCKET_TYPE_OBJECT_SEMANTIC) {
        // Fill in virtual inode if missing
        if (data->inodeNo <= 0) {
            data->inodeNo = generate_virtual_inode(key.c_str());
        }
        // Fill in dentryname if missing
        if (data->dentryname.empty()) {
            data->dentryname = get_basename(key);
        }
    }
...
void IndexCache::DeleteIndexWildcard(const std::string& pattern)
{
    if(pattern.empty()){
        S3FS_PRN_DBG("DeleteIndexWildcard called with empty pattern");
        return;
    }

    S3FS_PRN_INFO3("delete index cache entries with pattern[path=%s]", pattern.c_str());
    ...
    // 安全遍历并删除所有匹配的条目
    while(it != hashmap_.end()){
        // 前缀匹配: 删除所有以 pattern 开头的条目
        if(it->first.find(pattern) == 0){
            S3FS_PRN_DBG("deleting index cache entry[path=%s][inode=%lld]",
                        it->first.c_str(), it->second->data.inodeNo);

            // Remove from list
            SNAS_ListDel(&it->second->list_ptr);
            SNAS_ListAddTail(&it->second->list_ptr, &lru_head);
    ...
    S3FS_PRN_INFO3("deleted %zu index cache entries matching pattern[path=%s]",
...
void IndexCache::resizeMetaCacheCapacity(size_t capacity)
{
    ...
    extend_arrays_.push_back(extendEntries);  // save pointer for cleanup in destructor
