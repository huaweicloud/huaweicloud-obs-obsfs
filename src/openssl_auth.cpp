...
//-------------------------------------------------------------------
// [ISSUE2026040900001 B1] OpenSSL 1.1.0+ takes care of thread safety
// internally; the application no longer registers locking callbacks. The
// CRYPTO_set_*_callback macros expand to ((void)0) under 1.1+, so the static
// callback functions below have their addresses never taken at compile time
// and GCC reports them as -Wunused-function.
//
// Wrap the entire OpenSSL 1.0 compatibility block in a version check so:
//   - OpenSSL 1.1+ build: the functions and registration calls are excluded
//     entirely from compilation. s3fs_init_crypt_mutex / s3fs_destroy_crypt_mutex
//     become trivial `return true;` stubs (the same observable behavior as
//     before, since the registration was already a no-op).
//   - OpenSSL 1.0 build: the functions and registration calls are compiled
//     and executed exactly as before.
...
unsigned char* s3fs_md5hexsum_hws_obs(const char* buf, off_t start, size_t size)
{
  MD5_CTX       md5ctx;
  unsigned char* result;

  // [ISSUE2026040800001 M4] NULL check MUST precede any dereference (including
  // S3FS_PRN_DBG "%s"). Also do not log buf contents — if caller passes a
  // secret buffer with size=0, %s would leak plaintext into DBG log (CWE-532).
  if (NULL == buf){
    return NULL;
  }
  if (0 == size || 0 > start){  // size_t is unsigned, use == 0
  ...
  /************************************************/
  size_t        bytes;
  for(size_t total = 0; total < size; total += bytes){
    bytes = 512 < (size - total) ? 512 : (size - total);
    MD5_Update(&md5ctx, buf + start + total, bytes);
  }
  if(NULL == (result = (unsigned char*)malloc(get_md5_digest_length()))){
    return NULL;
  }
...
unsigned char* s3fs_sha256hexsum_hws_obs(const char* buf, off_t start, size_t size)
{
  const EVP_MD*  md = EVP_get_digestbyname("sha256");
  ...
  // [ISSUE2026040800001 M5] NULL check前置（对称修复 M4）— 本函数的 DBG log 本来
  // 就没打印 buf，所以缺陷 2a/2b 都不命中；但代码形状错仍应对齐，防止将来有人
  // 参考这里写新的对称函数时复制 bug。
  if (NULL == buf){
    return NULL;
  }
  if (0 == size || 0 > start){  // size is size_t (unsigned), use == 0
    S3FS_PRN_DBG("[start=%jd],[size=%lu]", (intmax_t)start, size);
    return NULL;
  }
  ...
  for(size_t total = 0; total < size; total += bytes){
    bytes = 512 < (size - total) ? 512 : (size - total);
    EVP_DigestUpdate(sha256ctx, buf+total+start, bytes);
  }
