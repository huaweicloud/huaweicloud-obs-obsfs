...
// Forward declarations for fdcache types (avoid circular include)
struct fdpage;
...
{
  private:
    BodyData(const BodyData&) = delete;             // non-copyable
    BodyData& operator=(const BodyData&) = delete;  // non-copyable
    char* text;
    size_t lastpos;
    size_t bufsize;
...
private:
  ...
  // [ISSUE2026040800002] mLock removed: a pthread_mutex_t member that was
  // init'd in Init() and destroyed in Destroy() but never lock()/unlock()'d
  // anywhere. The pool's actual concurrency control uses spinmLock below
  // (added by file gateway). The mutex was a historical residue from an
  // earlier mutex-based version.
/* file gateway modify begin */
  pthread_spinlock_t spinmLock;
/* file gateway modify end */
  CURL** mHandlers;
  int mIndex;
};
...
class S3fsCurl
{
  ...
  private:
    S3fsCurl(const S3fsCurl&) = delete;             // non-copyable
    S3fsCurl& operator=(const S3fsCurl&) = delete;  // non-copyable
...
/* file gateway modify begin */
    static pthread_spinlock_t  curl_handles_spinlock;
	static pthread_rwlock_t curl_aksk_rwlock;
/* file gateway modify end */
    ...
    static sse_type_t       ssetype;        //SSE_DISABLE default
    ...
    std::string          b_copy_source;        // backup for CopyMultipartPost retrying
    std::string          b_copy_source_range;  // backup for CopyMultipartPost retrying
    std::string          op;                   // the HTTP verb of the request ("PUT", "GET", etc.)
    std::string          query_string;         // request query string

/* file gateway modify begin */
    tagDataBuffer        writedatabuf;        // backup for retrying
    char*                read_buf;
    off_t                read_startpos;        // backup for retrying
    size_t               read_size;            // backup for retrying
    size_t               cur_pos;
/* file gateway modify end */
...
/* file gateway modify begin */
    static size_t ReadUploadCache(void* ptr, size_t blockSize, size_t numBlocks, void* data); 
/* file gateway modify end */
...
/* file gateway modify begin */
    CURLcode curl_easy_perform_mock(CURL* hCurl, long* LastResponseCode);
    std::string PrintRequestId4ResponseErr();
    int  getRootInodeNo(void);
/* file gateway modify end */
  ...
  public:
    ...
    static int MixMultipartUploadRequest(const char* tpath, headers_t& meta, int fd,
    ...
    // Bucket type detection (public for testing)
    static bool DetectBucketTypeFromHeaders(const headers_t& headers);
    ...
    CURLcode Hws_curl_easy_perform();
    int RequestPerform(void);
    int DeleteRequest(const char* tpath);
/* file gateway modify begin */
    bool PreHeadRequest(const char* tpath, const char* bpath = NULL, const char* savedpath = NULL, int ssekey_pos = -1, long long *inode_no = NULL, const char* shardkey = NULL);
    bool PreHeadRequest(std::string& tpath, std::string& bpath, std::string& savedpath, int ssekey_pos = -1, long long *inode_no = NULL, const char* shardkey = NULL) {
      return PreHeadRequest(tpath.c_str(), bpath.c_str(), savedpath.c_str(), ssekey_pos, inode_no, shardkey);
    }
    int HeadRequest(const char* tpath, headers_t& meta, long long* inode_no = NULL, const char* shardkey = NULL);
    ...
    int MultipartUploadRequest(const std::string& upload_id, const char* tpath, int fd, off_t offset, size_t size, etaglist_t& list, int etagpos, int part_num);
    int MultipartRenameRequest(const char* from, const char* to, headers_t& meta, off_t size);
/* file gateway modified begin */
    int CreateZeroByteFileObject(const char* tpath, headers_t& meta);
    int WriteBytesToFileObject(const char    *tpath, 
                               tagDataBuffer *databuff,
                               off_t          offset,
                               tag_index_cache_entry_t *fileMeta);
    void RenameSetopt();
    int RenameFileOrDirObject(const char *from, const char *to, bool& need_check_to);
    int CreateZeroByteDirObject(const char* tpath, headers_t& meta);
    int PreReadFromFileObject(const char* tpath,
                                        char* buf,
                                        off_t start,
                                        size_t size,
                                        sse_type_t ssetype,
    ...
    int ReadFromFileObject(const char* tpath, char* buf, off_t start, size_t size, const char *shardkey, const char *inodeNoStr);
    static size_t DownloadWriteCallbackforRead(void* ptr, size_t size, size_t nmemb, void* userp);
    int  getMountPrefixInode(void);
    // Object bucket mount prefix existence check (S3 standard, no x-hws-fs-* headers)
    // Returns 0 if the prefix exists (has any object/sub-prefix) or is empty (root).
    // Returns non-zero on failure or when the prefix is not found.
    int  CheckObjectBucketPrefixExists(const std::string& prefix);
/* file gateway modified end */

    // methods(variables)
    const CURL* GetCurlHandle(void) const { return hCurl; }  // const: callers should not modify handle
    ...
    const headers_t* GetResponseHeaders(void) const { return &responseHeaders; }  // const: callers only read
    const BodyData* GetBodyData(void) const { return bodydata; }  // const: callers only use str()/size()
    const BodyData* GetHeadData(void) const { return headdata; }  // const: no production code modifies
...
{
  private:
    S3fsMultiCurl(const S3fsMultiCurl&) = delete;             // non-copyable
    S3fsMultiCurl& operator=(const S3fsMultiCurl&) = delete;  // non-copyable
