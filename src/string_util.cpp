...
// Test support: weak symbol for controlling malloc failure in getMessage
// This allows tests to override behavior by defining this symbol
__attribute__((weak)) bool test_force_malloc_fail = false;
...
template std::string str(double value);
template std::string str(float value);
...
off_t s3fs_strtoofft(const char* str, bool is_base_16)
{
  ...
  for(result = 0, chk_space = false, chk_base16_prefix = false; '\0' != *str; str++){
    ...
    // [ISSUE2026040900002 B6] Fixed: was `'0' <= *str || '9' < *str` (always true
    // for any char >= '0', including letters). The || should be && and the upper
    // bound was inverted. This caused non-digit characters like 'G', 'K' to be
    // silently treated as digits (char - '0'), producing garbage values for
    // invalid input such as `-o cache_size=10G`.
    if('0' <= *str && *str <= '9'){
...
char* s3fs_base64(const unsigned char* input, size_t length)
{
  ...
  if(!input || 0 == length){  // size_t is unsigned, use == 0
    return NULL;
  }

  // [ISSUE2026040700002 task K] Defensive upper bound check.
  // The size formula `(length/3 + 1) * 4 + 1` overflows size_t when length is
  // close to SIZE_MAX*3/4 (~1.38 EB), which would malloc a small buffer and
  // then write beyond it in the encoding loop (CWE-190 → CWE-680 → CWE-787).
  // All current obsfs callers pass length ≤ 64 KB (xattr upper bound), so
  // this check never fires for any in-tree caller; it only blocks future
  // dangerous calls. 1 GB is chosen as a generous ceiling well above any
  // legitimate use case.
  static const size_t MAX_BASE64_INPUT = 1ULL << 30;
  if(length > MAX_BASE64_INPUT){
    S3FS_PRN_ERR("s3fs_base64: input length %zu exceeds maximum %zu",
                 length, MAX_BASE64_INPUT);
    return NULL;
  }
