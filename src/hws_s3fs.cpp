...
bool nomixupload                  = false;
bool streamread                   = false;
size_t streamread_window          = 24;
bool pathrequeststyle             = false;
bool complement_stat              = false;
bool accurate_readdir             = false;   // ISSUE2026051200001 M-4 review
...
bool nohwscache            = false;  /*default support cache*/

// Current bucket type (detected at mount time or manually specified)
bucket_type_t g_bucket_type = BUCKET_TYPE_UNKNOWN;

// DC-07: Track whether user explicitly set stat_cache_expire
static bool stat_cache_expire_explicitly_set = false;
// ISSUE2026051200001 M.4 review fix: same pattern as stat_cache_expire,
// avoids silently overriding a user who explicitly passed
// `-o max_stat_cache_size=1000` (e.g. embedded/low-memory deployments).
static bool max_stat_cache_size_explicitly_set = false;

// ISSUE2026051200001 M.4 review fix: extracted from main() so the unit
// test exercises the real code path (single source of truth — no
// duplicated lambda in the test fixture).
//
// Promote StatCache CacheSize to 100000 for object bucket unless the user
// explicitly set `-o max_stat_cache_size=N`. File bucket keeps the historical
// 1000 default regardless. Idempotent: safe to call multiple times.
static void apply_object_bucket_cache_size_default()
{
    if(BUCKET_TYPE_OBJECT_SEMANTIC == g_bucket_type && !max_stat_cache_size_explicitly_set){
        StatCache::getStatCacheData()->SetCacheSize(100000);
    }
}
...
static int set_and_encrypt_aksk(int argc, char* argv[]);
static int set_aksk(void* data, const char* arg, int key, struct fuse_args* outargs);
...
static int get_object_attribute_object_bucket(const char* path, struct stat* pstbuf, headers_t* pmeta, long* headLastResponseCode);
...
//
// Get object attributes for OBJECT bucket mode (S3-compatible, no IndexCache dependency)
// This aligns with s3fs-fuse behavior for standard S3
//
static int get_object_attribute_object_bucket(const char* path, struct stat* pstbuf, headers_t* pmeta, long* headLastResponseCode)
{
    S3FS_PRN_DBG("[object-bucket] get_object_attribute_object_bucket: path=%s", path);

    int result;
    headers_t meta_local;
    headers_t* meta = (pmeta ? pmeta : &meta_local);
    ...
    if(0 == strcmp(path, "/")){
        if(pstbuf){
            ...
            pstbuf->st_nlink = 2;
            pstbuf->st_mode = S_IFDIR | 0755;
            pstbuf->st_uid = getuid();
            pstbuf->st_gid = getgid();
            pstbuf->st_size = 4096;
            pstbuf->st_atime = pstbuf->st_mtime = pstbuf->st_ctime = time(NULL);
            pstbuf->st_ino = generate_virtual_inode(path);
        }
        return 0;
    }

    // Check StatCache first
    if(pstbuf){
        std::string cache_key = path;
        if(StatCache::getStatCacheData()->GetStat(cache_key, pstbuf, meta, true)){
            pstbuf->st_ino = generate_virtual_inode(path);
            return 0;
        }
    }

    // Query S3 with HEAD request
    S3fsCurl s3fscurl;
    long long responseCode_ll = headLastResponseCode ? *headLastResponseCode : 0;
    result = s3fscurl.HeadRequest(path, *meta, headLastResponseCode ? &responseCode_ll : NULL);
    if(headLastResponseCode) {
        *headLastResponseCode = static_cast<long>(responseCode_ll);
    }
    ...
    if(result != 0 && (result == -ENOENT || result == -2)){
        ...
        headers_t meta_tmp;
        long long headLastResponseCode_tmp = 0;
        int result2 = s3fscurl.HeadRequest(dir_path.c_str(), meta_tmp, &headLastResponseCode_tmp);

        if(result2 == 0){
            *meta = meta_tmp;
            if(headLastResponseCode){
                *headLastResponseCode = headLastResponseCode_tmp;
            }
            result = 0;
        }
    }
    ...
    // Convert headers to stat structure
    if(pstbuf){
        memset(pstbuf, 0, sizeof(struct stat));

        // Set inode from path hash (FNV-1a like s3fs-fuse)
        pstbuf->st_ino = generate_virtual_inode(path);

        // Get file size from Content-Length header
        headers_t::iterator it = meta->find("Content-Length");
        if(it != meta->end()){
            pstbuf->st_size = str_to_off_t(it->second.c_str());
        }

        // Get mode (default to regular file)
        pstbuf->st_mode = S_IFREG | 0644;
        pstbuf->st_nlink = 1;
        pstbuf->st_uid = getuid();
        pstbuf->st_gid = getgid();

        // Get modification time from Last-Modified header
        it = meta->find("Last-Modified");
        if(it != meta->end()){
            pstbuf->st_mtime = get_lastmodified(it->second.c_str());
        }
        pstbuf->st_atime = pstbuf->st_ctime = time(NULL);

        // Set block size and blocks
        pstbuf->st_blksize = 4096;
        pstbuf->st_blocks = (pstbuf->st_size + 511) / 512;
    }
...
{
    S3FS_PRN_DBG("[path=%s] g_bucket_type=%d", path, g_bucket_type);

    // Object bucket: use simplified logic without IndexCache dependency
    // This aligns with s3fs-fuse behavior for standard S3
    if(g_bucket_type == BUCKET_TYPE_OBJECT_SEMANTIC){
        return get_object_attribute_object_bucket(path, pstbuf, pmeta, headLastResponseCode);
    }

    // File bucket: use original logic with IndexCache
...
static int s3fs_readlink(const char* path, char* buf, size_t size)
{
    ...
    if(!path || !buf || 0 == size)  // size_t is unsigned, use == 0
    {
        return 0;
    }
...
static int s3fs_symlink(const char* from, const char* to)
{
    ...
    if (0 != result)
    {
		//1.delete cache 2.head again 3.retry write once more
...
static int rename_object(const char* from, const char* to)
{
  ...
  // Get inode and check if source exists - different approach for Object vs File Bucket
  bool get_ok = false;  // Track if IndexCache hit (File Bucket only)
  if(g_bucket_type == BUCKET_TYPE_FILE_SEMANTIC) {
      // File Bucket: Use IndexCache for inode lookup (preserves existing behavior)
      tag_index_cache_entry_t index_cache_entry;
      init_index_cache_entry(index_cache_entry);
      get_ok = IndexCache::getIndexCache()->GetIndex(from, &index_cache_entry);
      if (get_ok){
      ...
      // Object Bucket: Get stat to check if source exists (aligned with s3fs-fuse)
      if (0 != (result = get_object_attribute(from, &statBuf, &pmeta))) {
          S3FS_PRN_ERR("[from=%s]The src of rename is not existed , return err", from);
          return -EIO;
      }
      // Generate virtual inode for object bucket
      inodeNo = generate_virtual_inode(from);
  }
  //flush src data to osc
  //if not found,then file not open,no need to flush
  if(g_bucket_type == BUCKET_TYPE_FILE_SEMANTIC) {
      // File Bucket: Only flush if found in IndexCache (existing behavior)
      if (get_ok) {
      ...
      // [DEAD CODE] This branch is unreachable: rename_object() is only called
      // from s3fs_rename(), which is registered for file bucket only (init_oper L6616).
      // Object bucket uses s3fs_rename_s3 → rename_object_s3 instead.
      // Kept for clarity; GetFdEntity without Dup is unsafe but moot here.
      // See ISSUE2026031500003 audit item C1.
      FdEntity* ent = FdManager::get()->GetFdEntity(from, -1);
      if (ent) {
          ent->Flush();
      }
  }
  ...
  if(g_bucket_type == BUCKET_TYPE_FILE_SEMANTIC) {
      // File Bucket: Delete from IndexCache
      IndexCache::getIndexCache()->DeleteIndex(toString(from));
  }
  // Object Bucket: StatCache will be invalidated after successful rename (see below)
  ...
  {
    if(g_bucket_type == BUCKET_TYPE_OBJECT_SEMANTIC) {
      // Object Bucket: Update FdManager and invalidate StatCache (aligned with s3fs-fuse)
      FdManager::get()->Rename(from, to);
      S3FS_PRN_DBG("[object-bucket] FdManager rename cache updated from=%s to=%s", from, to);
      // Invalidate StatCache - getattr will rebuild cache on next access
      StatCache::getStatCacheData()->DelStat(from);
    } else {
      // File Bucket: Update IndexCache (preserves existing behavior)
...
static int s3fs_rename(const char* from, const char* to)
{
  ...
  // OBS supports Copy API, so rename works even in nocopyapi/norenameapi mode
  // The nocopyapi flag was originally for storage systems that don't support Copy API
  // Since OBS supports Copy API, we can use the same rename logic
                ...
                int             list_max_key)
{
  ...
  S3fsCurl  s3fscurl;
  xmlDocPtr doc;
  const BodyData* body = nullptr;  // const: only read via str()/size()
...
static int s3fs_listxattr(const char* path, char* list, size_t size)
{
  ...
  // check parameters
  if(size == 0){  // size_t is unsigned, use == 0
    free_xattrs(xattrs);
    return total;
  }
  if(!list || size < total){
...
static int obsfs_write_successfile()
{
    ...
    if (success_file_dir.empty()){  // size_t is unsigned, use empty()
       return 0;
    }
...
static int s3fs_check_service(const std::string& ak, const std::string& sk, const std::string& token)
{
  ...
  S3fsCurl s3fscurl;
  int      res;
  S3FS_PRN_WARN("s3fs_check_service: calling CheckBucket (host=%s, bucket=%s, bucket_type=%d)...", host.c_str(), bucket.c_str(), g_bucket_type);
  if(0 > (res = s3fscurl.CheckBucket(ak, sk, token))){
    // get response code
    long responseCode = s3fscurl.GetLastResponseCode();
    S3FS_PRN_WARN("s3fs_check_service: CheckBucket returned %d, HTTP %ld", res, responseCode);
    ...
    if(0 > res && responseCode != 200 && responseCode != 301){
      if(responseCode == 400){
        ...
        // HTTP 405 on bucket root is expected for object semantic buckets
        // This is not a fatal error - CheckBucket already handles this
        S3FS_PRN_INFO("Received HTTP 405 during service check - object semantic bucket");
        S3FS_PRN_INFO("Continuing with mount - individual object operations will work");
  ...
  // Three-way dispatch by g_bucket_type:
  //   FILE_SEMANTIC   -> file-bucket inode + dir check (depends on x-hws-fs-* headers)
  //   OBJECT_SEMANTIC -> S3-standard prefix existence check (no x-hws-fs-* headers)
  //   UNKNOWN         -> fail-fast (caller must specify -o bucket_type=file|object)
  // See ISSUE2026042900001.
  if(mount_prefix.size() > 0){
    ...
    } else if(g_bucket_type == BUCKET_TYPE_OBJECT_SEMANTIC){
      if(s3fscurl.CheckObjectBucketPrefixExists(mount_prefix) != 0){
        S3FS_PRN_CRIT("remote mountpath %s not found in object bucket.", mount_prefix.c_str());
        std::string errorMsg = std::string("remote mountpath ") + mount_prefix + " not found in object bucket.";
        HwsCipherKeyCheck::GetInstance().updateFail(errorMsg.c_str());
        return EXIT_FAILURE;
      }
    } else {
      S3FS_PRN_CRIT("bucket type unknown, cannot validate mount_prefix %s. "
                    "Please specify -o bucket_type=file|object explicitly.",
                    mount_prefix.c_str());
      std::string errorMsg = std::string("bucket type unknown, cannot validate mount_prefix ") +
                              mount_prefix + ". Please specify -o bucket_type=file|object explicitly.";
      HwsCipherKeyCheck::GetInstance().updateFail(errorMsg.c_str());
      return EXIT_FAILURE;
    }
  }
...
static int parse_passwd_file(bucketkvmap_t& resmap)
{
  ...
  for(iter = linelist.begin(); iter != linelist.end(); ++iter){
    if (0 == iter->compare(HwsEncryption::get_instance().encrypted_flag)) {
      isEncrypted = true;
      S3FS_PRN_INFO("aksk is encrypted");
      continue;
    }
    ...
    if(isEncrypted){
      // encrypt case
      string decode_accesskey;
      string decode_secretkey;
      string decode_token;
      if(HwsEncryption::get_instance().genRootKeyAndDecryptData(accessKey, accessKey, decode_accesskey) != 0){
        HwsCipherKeyCheck::GetInstance().updateFail("Decryption failed.");
        return -1;
      }
      if(HwsEncryption::get_instance().genRootKeyAndDecryptData(accessKey, secretKey, decode_secretkey) != 0){
        HwsCipherKeyCheck::GetInstance().updateFail("Decryption failed.");
        return -1;
      }
      if(!token.empty() && HwsEncryption::get_instance().genRootKeyAndDecryptData(accessKey, token, decode_token) != 0){
        HwsCipherKeyCheck::GetInstance().updateFail("Decryption failed.");
        return -1;
      }
      kv[string(aws_accesskeyid)] = decode_accesskey;
      kv[string(aws_secretkey)] = decode_secretkey;
      kv[string(aws_token)] = decode_token;
    } else {
      // plaintext case - backward compatibility
...
static int my_fuse_opt_proc(void* data, const char* arg, int key, struct fuse_args* outargs)
{
  ...
  if(key == FUSE_OPT_KEY_NONOPT){
    ...
    if(0 == STR2NCMP(arg, "default_acl=")){
      ...
      if('\0' != *acl &&
         0 != strcmp(acl, "private") &&
         0 != strcmp(acl, "public-read") &&
         0 != strcmp(acl, "public-read-write") &&
         0 != strcmp(acl, "authenticated-read") &&
         0 != strcmp(acl, "bucket-owner-read") &&
         0 != strcmp(acl, "bucket-owner-full-control") &&
         0 != strcmp(acl, "log-delivery-write"))
      {
        S3FS_PRN_EXIT("unknown value for default_acl: %s", acl);
        return -1;
      }
    ...
    if(0 == STR2NCMP(arg, "max_dirty_data=")){
      const char* value = strchr(arg, '=') + sizeof(char);
      // s3fs_strtoofft doesn't handle negative numbers, so check for "-1" explicitly
      off_t size;
      if(0 == strcmp(value, "-1")){
        size = -1;
      }else{
        size = s3fs_strtoofft(value);
        if(size < 50){
          S3FS_PRN_EXIT("max_dirty_data option must be at least 50 (MB) or -1 to disable.");
          return -1;
        }
        size *= 1024 * 1024;  // MB to bytes
      }
      FdManager::SetMaxDirtyData(size);
      return 0;
    }
    ...
    if(0 == STR2NCMP(arg, "multireq_max=")){
      ...
      if(0 >= maxreq){
        S3FS_PRN_EXIT("argument should be over 1: multireq_max");
        return -1;
      }
    ...
    if(0 == strcmp(arg, "nomultipart")){
      nomultipart = true;
      nomixupload = true;   // no multipart means mix is impossible
      return 0;
    }
    ...
    if(0 == strcmp(arg, "streamread")){
      streamread = true;
      return 0;
    }
    if(0 == STR2NCMP(arg, "streamread_window=")){
      const char* value = strchr(arg, '=') + sizeof(char);
      off_t size = s3fs_strtoofft(value);
      if(size < 24){
        S3FS_PRN_EXIT("streamread_window must be at least 24 (MB).");
        return -1;
      }
      if(size > 4096){
        S3FS_PRN_EXIT("streamread_window must be at most 4096 (MB).");
        return -1;
      }
      streamread_window = static_cast<size_t>(size);
      return 0;
    }
    if(0 == STR2NCMP(arg, "bucket_type=")){
      const char* type = strchr(arg, '=') + sizeof(char);
      if(0 == strcmp(type, "object")) {
        g_bucket_type = BUCKET_TYPE_OBJECT_SEMANTIC;
        nohwscache = true;  // Object semantic mode uses direct OBS write, not HwsFdManager
        S3FS_PRN_INFO("Bucket type manually set to OBJECT_SEMANTIC (nohwscache enabled)");
      } else if(0 == strcmp(type, "file")) {
        g_bucket_type = BUCKET_TYPE_FILE_SEMANTIC;
        S3FS_PRN_INFO("Bucket type manually set to FILE_SEMANTIC");
      } else {
        S3FS_PRN_EXIT("invalid bucket_type: %s (must be 'object' or 'file')", type);
        return -1;
      }
      return 0;
    }
    ...
    if(0 == STR2NCMP(arg, "public_bucket=")){
      ...
      if(1 == pubbucket){
        ...
        //
        nocopyapi = true;
        nomixupload = true;   // CopyPart not available, disable mixmultipart
      }else if(0 == pubbucket){
    ...
    if(0 == STR2NCMP(arg, "max_stat_cache_size=")){
      ...
      if(cache_size > 1000000){
        S3FS_PRN_WARN("max_stat_cache_size=%lu is very large, may consume excessive memory.", cache_size);
      }
    ...
    if(0 == STR2NCMP(arg, "stat_cache_expire=")){
      ...
      if(expr_time < 0){
        S3FS_PRN_EXIT("stat_cache_expire must be a non-negative value (seconds).");
        return -1;
      }
      StatCache::getStatCacheData()->SetExpireTime(expr_time);
      stat_cache_expire_explicitly_set = true;
      return 0;
    }
    // [NOTE]
    ...
    if(0 == STR2NCMP(arg, "stat_cache_interval_expire=")){
      time_t expr_time = static_cast<time_t>(s3fs_strtoofft(strchr(arg, '=') + sizeof(char)));
      if(expr_time < 0){
        S3FS_PRN_EXIT("stat_cache_interval_expire must be a non-negative value (seconds).");
        return -1;
      }
    ...
    if(0 == STR2NCMP(arg, "max_tmpdir_disk_usage=")){
      off_t raw = s3fs_strtoofft(strchr(arg, '=') + sizeof(char));
      if(raw < 0){
        S3FS_PRN_EXIT("max_tmpdir_disk_usage must be a non-negative value (MB).");
        return -1;
      }
      if(raw > static_cast<off_t>(SIZE_MAX / (1024 * 1024))){
        S3FS_PRN_EXIT("max_tmpdir_disk_usage value too large: %lld MB", (long long)raw);
        return -1;
      }
      size_t msize = static_cast<size_t>(raw) * 1024 * 1024;
      FdManager::SetMaxTmpdirDiskUsage(msize);
      return 0;
    }
    if(0 == STR2NCMP(arg, "singlepart_copy_limit=")){
      off_t raw = s3fs_strtoofft(strchr(arg, '=') + sizeof(char));
      if(raw < 0){
        S3FS_PRN_EXIT("singlepart_copy_limit must be a non-negative value (KB).");
        return -1;
      }
      singlepart_copy_limit = static_cast<int64_t>(raw) * 1024;
      return 0;
    }
    ...
    // ISSUE2026051200001 M-4 review fix
    if(0 == strcmp(arg, "accurate_readdir")){
    ...
    if(0 == STR2NCMP(arg, "url=")){
      ...
      if(host.substr(0, 7) != "http://" && host.substr(0, 8) != "https://"){
        S3FS_PRN_EXIT("url must start with http:// or https:// : %s", host.c_str());
        return -1;
      }
...
static int set_and_encrypt_aksk(int argc, char* argv[])
{
  ...
  if(0 != fuse_opt_parse(&custom_args, NULL, NULL, set_aksk)){
    S3FS_PRN_EXIT("fuse_opt_parse fail");
    return -1;
  }

  if(0 != HwsEncryption::get_instance().encryptAkAndSkToPasswdFile(accesskey,secretkey, securitytoken,passwd_file)){
    S3FS_PRN_EXIT("set ak sk passwd_file failed");
    return -1;
  } else{
    S3FS_PRN_EXIT("set ak sk passwd_file");
    return 0;
  }
}
//校验ak、sk、token、passwd_path长度
static int check_keysize(string key,const char* arg)
{
  string resultKey = strchr(arg, '=') + sizeof(char);
  //截取=号前边的名称
  string titleKey= key.substr(0,key.find_first_of('='));
  if (resultKey.size() == 0) {
    S3FS_PRN_EXIT("%s size is invalid.", titleKey.c_str());
    return -1;
  }
  return 1;
...
static int set_aksk(void* data, const char* arg, int key, struct fuse_args* outargs)
{
  ...
  if (0 == STR2NCMP(arg,"ak=")) {
    accesskey = strchr(arg, '=') + sizeof(char);
    return check_keysize("ak=",arg);
  }
  if (0 == STR2NCMP(arg,"sk=")) {
    secretkey = strchr(arg, '=') + sizeof(char);
    return check_keysize("sk=",arg);
  }
  if (0 == STR2NCMP(arg,"token=")) {
    securitytoken = strchr(arg, '=') + sizeof(char);
    return check_keysize("token=",arg);
  }
  if (0 == STR2NCMP(arg,"passwd_file=")) {
    struct stat passwd_stat;
    passwd_file = strchr(arg, '=') + sizeof(char);
    if(stat(passwd_file.c_str(), &passwd_stat) == 0) {
        S3FS_PRN_EXIT("passwd_file %s already exists.", passwd_file.c_str());
        return -1;
    }
    return check_keysize("passwd_file=",arg);
  }
  return 1;
}
...
int GetIndexCacheEntryWithRetry(string path,tag_index_cache_entry_t* pIndexEntry)
{
    ...
    S3FS_PRN_DBG("[TRACE] GetIndexCacheEntryWithRetry called: path=%s, g_bucket_type=%d", path.c_str(), g_bucket_type);
...
static void  update_maxDhtViewVersion_from_response(const char* path, const headers_t *response_headers)  // const: read-only access
{
    ...
    const headers_t *header = response_headers;
    for(headers_t::const_iterator iter = header->begin(); iter != header->end(); ++iter){
...
{
    int          result = -1;  // removed debug log with raw pointer addresses
    ...
    if(g_bucket_type == BUCKET_TYPE_FILE_SEMANTIC && useIndexCache) {
        // File Bucket: Use IndexCache
        get_ok = IndexCache::getIndexCache()->GetIndex(strpath, p_cache_entry);
    }
    ...
    if (get_ok)
    {
        // File Bucket: Use IndexCache logic
        if (useIndexCache)
        {
    ...
    {
        // Object Bucket or IndexCache miss: Direct HeadRequest
        result = s3fscurl.HeadRequest(strpath.c_str(), (*pheader));
    }
    ...
    if (INVALID_INODE_NO != p_cache_entry->inodeNo)
    {
        ...
        // Object Bucket: StatCache already handles this (aligned with s3fs-fuse)
...
static int s3fs_open_hw_obs(const char* path, struct fuse_file_info* fi)
{
    ...
    if((unsigned int)fi->flags & O_TRUNC)
    {
        ...
        // Update st.st_size to 0 after truncate for correct fd entity size
        st.st_size = 0;
    }
...
/**
 * Object bucket write operation - simplified S3 upload for cache flush
 *
 * Key differences from s3fs_write_hw_obs_proc:
 * - Does NOT require IndexCache lookup (file may not exist in IndexCache yet)
 * - Does NOT require OBS-specific headers (dentryname, inodeno)
 * - Simpler retry logic without IndexCache dependencies
 * - Updates StatCache instead of IndexCache
 * - Works with standard S3 providers (AWS, MinIO, Ceph, etc.)
 *
 * NOTE: This is called from sendto_obs when flushing cached pages.
 * For object bucket, pages are flushed with full object replacement.
 *
 * @param path File path to write
 * @param buf Data buffer to write
 * @param size Number of bytes to write
 * @param offset Offset in file (for simplicity, we do full upload at offset 0)
 * @return Number of bytes written on success, negative error code on failure
 */
int s3fs_write_object_proc(const char* path, const char* buf, size_t size, off_t offset)
{
    std::string str_path = path;
    S3FS_PRN_INFO("[object-bucket] s3fs_write_object_proc: path=%s, size=%zu, offset=%ld",
                 path, size, (long)offset);

    // For object bucket, we use the existing s3fs_write_hw_obs_proc
    // but ensure the file exists in IndexCache first
    // This provides compatibility with the existing cache flush mechanism

    S3fsCurl s3fscurl(true);
    tag_index_cache_entry_t index_cache_entry;
    init_index_cache_entry(index_cache_entry);

    // Ensure file exists in IndexCache
    int getIndexCacheResult = IndexCache::getIndexCache()->GetIndex(str_path, &index_cache_entry);

    if (0 != getIndexCacheResult) {
        // File does not exist in IndexCache, need to create it first
        S3FS_PRN_INFO("[object-bucket] File not in IndexCache, creating: %s", path);

        // Build standard metadata for new file
        headers_t meta;
        build_standard_metadata(meta, size, S3fsCurl::LookupMimeType(path), true);

        // Create empty file via PUT request
        int createResult = s3fscurl.PutRequest(path, meta, -1);
        if (createResult != 0) {
            S3FS_PRN_ERR("[object-bucket] Failed to create file: %s (result=%d)", path, createResult);
            return createResult;
        }

        // Try to get IndexCache entry again
        getIndexCacheResult = IndexCache::getIndexCache()->GetIndex(str_path, &index_cache_entry);
        if (0 != getIndexCacheResult) {
            // For object bucket, initialize with default values if IndexCache still fails
            S3FS_PRN_WARN("[object-bucket] IndexCache not available after create, using defaults");
            index_cache_entry.inodeNo = 0;
        }
    }

    // Now use WriteBytesToFileObject (same as file bucket)
    tagDataBuffer databuff;
    databuff.pcBuffer     = const_cast<char*>(buf);
    databuff.ulBufferSize = size;
    databuff.offset       = 0;
    ...
    for(int i = 0; i < HWS_RETRY_WRITE_OBS_NUM; i++)
    {
        ...
        if (0 == writeResult)
        {
            S3FS_PRN_INFO("[object-bucket] Write successful: path=%s, size=%zu, offset=%ld",
                         str_path.c_str(), size, (long)offset);

            // Update StatCache for Object bucket mode
            StatCache::getStatCacheData()->DelStat(str_path);

            return static_cast<int>(size);
        }
        else
        {
            S3FS_PRN_ERR("[object-bucket] Write failed, retrying: path=%s, result=%d, retry=%d/%d",
                        str_path.c_str(), writeResult, i, HWS_RETRY_WRITE_OBS_NUM);
            sleep(4);
        }
    }

    // All retries failed
    S3FS_PRN_ERR("[object-bucket] Write failed after all retries: path=%s, result=%d", str_path.c_str(), writeResult);
    return -EIO;
}
...
{
    // DEBUG: Log to confirm which write function is called
    S3FS_PRN_DBG("[TRACE] s3fs_write_hw_obs CALLED: path=%s, size=%zu, g_bucket_type=%d",
                 path, size, g_bucket_type);
    ...
    if (get_ok)
    {
        ...
        // For Object Bucket, fi->fh is the FdEntity's fd, not the inode
        // So we skip this check for Object Bucket
        if(g_bucket_type != BUCKET_TYPE_OBJECT_SEMANTIC &&
           fi->fh != (unsigned long long)index_cache_entry.inodeNo)
        {
    ...
    S3FS_PRN_INFO("s3fs_write_hw_obs: nohwscache=%d, path=%s, size=%zu, offset=%jd",
                  nohwscache, path, size, (intmax_t)offset);
...
static void add_list_result_to_filler(const char* path, string dentryPrefix, S3HwObjList& head, void* buf, off_t offset, fuse_fill_dir_t filler, PAGE_FILL_STATE& fill_state, string& next_marker)
{
    ...
    for (auto it = head.begin(); it != head.end(); it++){
        ...
        // Update IndexCache for file bucket only
        if(g_bucket_type == BUCKET_TYPE_FILE_SEMANTIC) {
...
static int s3fs_mknod_hw_obs(const char *path, mode_t mode, dev_t rdev)
{
  int validateLengthResult = validate_filepath_length(path);
  if( validateLengthResult != 0) {
    S3FS_PRN_ERR("mknod: target path length exceeds maximum, abort [path=%s][result=%d]", path, validateLengthResult);
    return validateLengthResult;
  }
...
{
    S3FS_PRN_INFO("=== init_oper CALLED: g_bucket_type=%d (OBJECT=%d, FILE=%d, UNKNOWN=%d) ===",
        g_bucket_type, BUCKET_TYPE_OBJECT_SEMANTIC, BUCKET_TYPE_FILE_SEMANTIC, BUCKET_TYPE_UNKNOWN);

    // Select operation implementations based on bucket type
    if (g_bucket_type == BUCKET_TYPE_OBJECT_SEMANTIC) {
        // Object semantic mode (standard S3)
        S3FS_PRN_INFO("Initializing OBJECT_SEMANTIC mode operations (standard S3)");

        s3fs_oper.getattr   = s3fs_getattr_s3;
        s3fs_oper.readlink  = s3fs_readlink_s3;
        s3fs_oper.mknod     = s3fs_mknod_s3;
        s3fs_oper.mkdir     = s3fs_mkdir_s3;
        s3fs_oper.unlink    = s3fs_unlink_s3;
        s3fs_oper.rmdir     = s3fs_rmdir_s3;
        s3fs_oper.symlink   = s3fs_symlink_s3;
        s3fs_oper.rename    = s3fs_rename_s3;
        s3fs_oper.link      = s3fs_link_s3;

        if(!nocopyapi){
            s3fs_oper.chmod   = s3fs_chmod_s3;
            s3fs_oper.chown   = s3fs_chown_s3;
            s3fs_oper.utimens = s3fs_utimens_s3;
        }else{
            s3fs_oper.chmod   = s3fs_chmod_nocopy_s3;
            s3fs_oper.chown   = s3fs_chown_nocopy_s3;
            s3fs_oper.utimens = s3fs_utimens_nocopy_s3;
        }

        s3fs_oper.truncate  = s3fs_truncate_s3;
        s3fs_oper.open      = s3fs_open_s3;
        s3fs_oper.read      = s3fs_read_s3;
        s3fs_oper.write     = s3fs_write_s3;
        s3fs_oper.statfs    = s3fs_statfs_s3;
        s3fs_oper.flush     = s3fs_flush_s3;
        s3fs_oper.fsync     = s3fs_fsync_s3;
        s3fs_oper.release   = s3fs_release_s3;
        s3fs_oper.opendir   = s3fs_opendir_s3;
        s3fs_oper.readdir   = s3fs_readdir_s3;
        s3fs_oper.access    = s3fs_access_s3;
        s3fs_oper.create    = s3fs_create_s3;
        // removed debug log with function pointer address (no diagnostic value)

        // extended attributes
        if(is_use_xattr){
            s3fs_oper.setxattr    = s3fs_setxattr_s3;
            s3fs_oper.getxattr    = s3fs_getxattr_s3;
            s3fs_oper.listxattr   = s3fs_listxattr_s3;
            s3fs_oper.removexattr = s3fs_removexattr_s3;
        }
    } else {
        // File semantic mode (OBS file semantic, or UNKNOWN - default to OBS)
        S3FS_PRN_INFO("Initializing FILE_SEMANTIC mode operations (OBS file semantic)");
    ...
    // Common operations (same for both modes)
...
int main(int argc, char* argv[])
{
  ...
  static const struct option long_opts[] = {
    ...
    {"password_encryption",  optional_argument, 0, 0},
    {0, 0, 0, 0}
  };
  ...
  while((ch = getopt_long(argc, argv, "dho:fsu", long_opts, &option_index)) != -1){
    ...
    case 0:
      ...
      if(strcmp(long_opts[option_index].name, "password_encryption") == 0){
        int encyrpt_result = set_and_encrypt_aksk(argc, argv);
        return encyrpt_result;
      }
      break;
    case 'h':
      show_help();
  ...
  // IP-based endpoints cannot use virtual-hosted style (bucket.192.168.1.1 is not valid DNS)
  if(!pathrequeststyle && is_ip_address_endpoint(host)){
    S3FS_PRN_WARN("endpoint is an IP address(%s), virtual-hosted style is not available, switching to path request style automatically.", host.c_str());
    pathrequeststyle = true;
  }
  ...
  if(!FdManager::CheckTmpDirExist()){
    S3FS_PRN_EXIT("temporary directory(%s) doesn't exist.", FdManager::GetTmpDir());
    exit(EXIT_FAILURE);
  }

  // check tmpdir filesystem type supports punch hole (R-20)
  if(!FdManager::CheckTmpDirFsType()){
    exit(EXIT_FAILURE);
  }
  ...
  */
  ...
  // Check Bucket
  {
      ...
      if(!S3fsCurl::GetAccessKeyAndToken(strAK, strSK, strToken)){
          S3FS_PRN_WARN("failed to Get access key/secret key from curl.");
          exit(EXIT_FAILURE);
      }
      int result;
      if (EXIT_SUCCESS != (result = s3fs_check_service(strAK, strSK, strToken))) {
          exit(result);
      }
      S3FS_PRN_WARN("CheckBucket passed, bucket_type=%d, pathrequeststyle=%d", g_bucket_type, pathrequeststyle);
  }

  init_oper(s3fs_oper);

  // DC-07: Default stat_cache_expire=900s for object bucket (s3fs-fuse compatible)
  if(BUCKET_TYPE_OBJECT_SEMANTIC == g_bucket_type && !stat_cache_expire_explicitly_set){
      StatCache::getStatCacheData()->SetExpireTime(900);
      S3FS_PRN_INFO("Object bucket: default stat_cache_expire=900s");
  }

  // ISSUE2026051200001 M.4: Default max_stat_cache_size=100000 for object
  // bucket (align with s3fs-fuse cache.cpp:39 `CacheSize(100'000)`).
  // After M.3 sets notruncate=false on all setter paths, the cache is
  // strictly bounded by CacheSize via LRU. Real-measured resident size
  // is ~120 MB at 100k entries (≈ 1.3 KB/entry with malloc overhead;
  // see docs/mount_option_spec.md for breakdown). File bucket keeps the
  // historical 1000 default (its workload patterns and IndexCache reliance
  // differ).
  // Only override when user has NOT explicitly set max_stat_cache_size,
  // tracked via max_stat_cache_size_explicitly_set (the previous value-
  // equality check incorrectly clobbered `-o max_stat_cache_size=1000`
  // from embedded/low-memory deployments). See apply_object_bucket_cache_size_default().
  apply_object_bucket_cache_size_default();
  if(BUCKET_TYPE_OBJECT_SEMANTIC == g_bucket_type && !max_stat_cache_size_explicitly_set){
      S3FS_PRN_INFO("Object bucket: default max_stat_cache_size=100000");
  }
  ...
  S3FS_PRN_WARN("All checks passed, calling fuse_main (mountpoint=%s)...", mountpoint.c_str());
