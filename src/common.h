...
// Bucket type detection for dual-mode support
enum bucket_type_t {
    BUCKET_TYPE_UNKNOWN = 0,        // Type not yet detected
    BUCKET_TYPE_OBJECT_SEMANTIC,    // Standard S3-compatible bucket
    BUCKET_TYPE_FILE_SEMANTIC       // OBS file semantic bucket
};

// Object type for StatCache child queries (Object bucket mode)
enum objtype_t {
    OBJTYPE_UNKNOWN    = -1,
    OBJTYPE_FILE       = 0,
    OBJTYPE_SYMLINK    = 1,
...
// Helper macros for checking object types
#define IS_FILE_OBJ(type)     (OBJTYPE_FILE == (type))
#define IS_SYMLINK_OBJ(type)  (OBJTYPE_SYMLINK == (type))
#define IS_DIR_OBJ(type)      (OBJTYPE_DIR == (type))

// Map of object name to object type (for StatCache child queries)
...
extern bool           nomixupload;
extern bool           streamread;
extern size_t         streamread_window;
extern bool           pathrequeststyle;
extern bool           complement_stat;
// ISSUE2026051200001 M-4 review: when true, s3fs_readdir_s3 does NOT
// populate StatCache with synthesized meta (M.1/M.2). Subsequent getattr
// falls back to per-entry HEAD, recovering s3fs-fuse `readdir_multi_head`
// accuracy for buckets where external SDK uploads set custom
// `x-amz-meta-uid` / `gid` / `mode`. Trade-off: N HEAD requests on the
// first ls of a large directory. Default off (use synthesized meta).
extern bool           accurate_readdir;
...
// Bucket type for dual-mode support
extern bucket_type_t g_bucket_type;

// Cache mode flag - when true, bypass HwsFdManager for direct OBS operations
extern bool nohwscache;

// Virtual inode helpers for S3 object semantic mode
// These are used to generate stable inode numbers from file paths
// when server-side inodes (OBS file semantic) are not available
