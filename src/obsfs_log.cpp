         ...
         va_list            ap)
{
    ...
    // Bounds check for g_sLogLevel array access (only 5 elements: 0-4)
    const char* level_str = (log_level < 5) ?
                             g_sLogLevel[log_level] : "UNKNOWN";
...
int32_t WriteCachetoDisk(uint32_t& time_count)
{
    ...
    if (g_write_disk_buf.offset == 0)  // unsigned, use == 0
    {
        return RET_OK;
    }
...
void ManualFlushLog(void)
{
...

    {
        ...
        if (0 != g_ping_buf.offset)
        {
            // [ISSUE2026040900001 A4] GCC 13+ rejects bare `(void)write(...)`
            // for functions with warn_unused_result. Best-effort log flush —
            // there is no recovery if write() fails (we are the log path).
            ssize_t write_ret = write(g_log_file, g_ping_buf.start, g_ping_buf.offset);
            (void)write_ret;
        ...
        if (0 != g_pong_buf.offset)
        {
            // [ISSUE2026040900001 A5] Same as A4 (pong buffer branch).
            ssize_t write_ret = write(g_log_file, g_pong_buf.start, g_pong_buf.offset);
            (void)write_ret;
