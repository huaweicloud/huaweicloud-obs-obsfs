...
mode_t get_mode(headers_t& meta, const char* path, bool checkdir, bool forcedir)
{
  ...
  if(meta.end() != (iter = meta.find(hws_obs_meta_mode))){
    ...
    if(meta.end() != (iter = meta.find("x-amz-meta-permissions"))){ // for s3sync
      mode = get_mode((*iter).second.c_str());
      isS3sync = true;
    }else if(BUCKET_TYPE_OBJECT_SEMANTIC == g_bucket_type){
      // ISSUE2026051200001 C.3: align with s3fs-fuse metaheader.cpp:147 —
      // when object has no mode metadata, default to POSIX-minimal sensible
      // mode (0750 for directories, 0640 for files) instead of leaving
      // mode=0 (which would yield 040000 after refine and break traversal).
      //
      // 0750/0640 matches s3fs-fuse upstream exactly. SMB integration that
      // needs world-traversal should use Samba `force user = <mount-user>`
      // (which maps guest sessions to the obsfs owner, who has full perms),
      // not loosen the filesystem default.
      //
      // Guard with object bucket only: file bucket has its own semantics
      // (complement_stat option may rely on mode==0 being detectable) and
      // is out of scope for this issue.
      bool is_dir = forcedir ||
                    (path && strlen(path) > 0 && '/' == path[strlen(path) - 1]);
      mode = is_dir ? 0750 : 0640;
    }
  }
...
uid_t get_uid(headers_t& meta)
{
  ...
  if(meta.end() == (iter = meta.find(hws_obs_meta_uid))){
    if(meta.end() == (iter = meta.find("x-amz-meta-owner"))){ // for s3sync
      // ISSUE2026051200001 C.2: object bucket only — align with s3fs-fuse
      // metaheader.cpp:325. Externally-uploaded objects without
      // x-amz-meta-uid would otherwise appear as root, breaking SMB guest
      // traversal. File bucket keeps the historical 0-fallback (its
      // ownership story is anchored on file gateway semantics, out of scope).
      if(BUCKET_TYPE_OBJECT_SEMANTIC == g_bucket_type){
        return geteuid();
      }
      return 0;
    }
...
gid_t get_gid(headers_t& meta)
{
  ...
  if(meta.end() == (iter = meta.find(hws_obs_meta_gid))){
    if(meta.end() == (iter = meta.find("x-amz-meta-group"))){ // for s3sync
      // ISSUE2026051200001 C.2: see get_uid above. Mirror behavior for gid.
      if(BUCKET_TYPE_OBJECT_SEMANTIC == g_bucket_type){
        return getegid();
      }
      return 0;
...
void show_help (void)
{
  ...
  printf(
    ...
    "\n"
    "   tmpdir (default=\"/tmp\")\n"
    "      - local folder for temporary files when not using cache.\n"
    "        useful when /tmp is small and large files need to be written.\n"
    "\n"
    "   max_dirty_data (default=\"5120\" i.e. 5GB)\n"
    "      - flush and reclaim disk when dirty data reaches this size (MB).\n"
    "        enables writing files larger than local disk space.\n"
    "        set to -1 to disable. minimum value is 50 (MB).\n"
    "\n"
    ...
    "\n"
    "   stat_cache_expire (default is 900 for object bucket, no expire for file bucket)\n"
    ...
    "\n"
    "   nomixupload (disable mixed multipart uploads, always upload full file)\n"
    "\n"
    "   streamread (enable async prefetch for sequential reads)\n"
    "\n"
    "   streamread_window (default=\"512\")\n"
    "      - size of each prefetch range in MB for streamread mode.\n"
...
//
// Virtual inode helpers for S3 object semantic mode
//

// Generate a stable virtual inode number from a file path
// Uses FNV-1a hash algorithm for good distribution and stability
ino_t generate_virtual_inode(const char* path)
{
    ...
    // Ensure result is positive and not INVALID_INO
    // INVALID_INO is typically 0, so we avoid that
    if (hash == 0 || hash == 1) {  // 1 is often used as root inode
        hash = 2;
    }

    // Clear the sign bit to ensure positive value
    return (ino_t)(hash & 0x7FFFFFFFFFFFFFFFULL);
}

// Extract the basename (last component) from a path
// Used to simulate dentryname for object semantic mode
...
// Check if an endpoint URL contains an IP address instead of a domain name.
// IP-based endpoints cannot use virtual-hosted style requests because
// "bucket.192.168.1.100" is not a valid DNS name.
// Handles: http://1.2.3.4, https://10.0.0.1:8080, http://192.168.1.1/path
bool is_ip_address_endpoint(const std::string& ep_url)
{
    // Strip scheme (http:// or https://)
    std::string host_part = ep_url;
    std::string::size_type scheme_end = host_part.find("://");
    if(scheme_end != std::string::npos){
        host_part = host_part.substr(scheme_end + 3);
    }

    // Strip port and path: take everything before ':' or '/'
    std::string::size_type pos = host_part.find_first_of(":/");
    if(pos != std::string::npos){
        host_part = host_part.substr(0, pos);
    }
    ...
    // Check if host_part is an IPv4 address: digits and dots only,
    // with exactly 3 dots and each octet 0-255
    int dots = 0;
    int octet = 0;
    bool has_digit = false;

    for(size_t i = 0; i < host_part.size(); i++){
        char c = host_part[i];
        if(c == '.'){
            ...
            return false;  // non-digit, non-dot → domain name
        }
    }

    // Final octet check
    if(!has_digit || octet > 255) return false;
