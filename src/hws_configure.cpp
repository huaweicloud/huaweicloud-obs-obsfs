...
//global variable
#ifdef HWS_CONFIGURE_TEST_PATH
const std::string g_logConfigurePath = HWS_CONFIGURE_TEST_PATH;
#else
...
{
    static bool config_notified = false;
    std::ifstream fp(g_logConfigurePath);
    if (false == fp.is_open())
    {
        // Only log once to avoid spamming logs when config file doesn't exist
        if (!config_notified) {
            S3FS_PRN_DBG("obsfsconfig not found at %s (this is optional)", g_logConfigurePath.c_str());
            config_notified = true;
        }
        return;
    }
    // Reset flag if file is found (in case it's created later)
...
int HwsConfigure::HwsGetIntConfigInClass(
        HwsConfigIntEnum  paramEnum)
{
    if (paramEnum < 0 || paramEnum >= HWS_CFG_INT_END)
    {
