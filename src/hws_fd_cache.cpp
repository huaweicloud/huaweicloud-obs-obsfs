...
#endif
// s3fs_write_hw_obs_proc, s3fs_read_hw_obs_proc, s3fs_write_object_proc
// declared in hws_fd_cache.h
...
size_t g_wholeFileCacheNotHitSizePerPeriod = 5 * HWS_MB_SIZE;
...
extern "C" const char* print_err_type(hws_write_err_type_e err_type){
    switch (err_type)
    {
        HWS_ENUM_TO_STR(CRC_ERR)
        HWS_ENUM_TO_STR(SEND_OBS_ERR)
        HWS_ENUM_TO_STR(BUTT_ERR)
    }
    return "invalid_err";
}
...
#else
    // 🔑 KEY MODIFICATION: Branch based on bucket type
    // This is the critical fix for Object bucket write operations
    if (g_bucket_type == BUCKET_TYPE_OBJECT_SEMANTIC) {
        // Object bucket: Use standard S3 write path
        // - No dependency on OBS headers (dentryname, inodeno)
        // - No dependency on IndexCache
        // - Uses standard S3 PutObject API
        // - Compatible with AWS S3, MinIO, Ceph, etc.
        S3FS_PRN_DBG("[object-bucket] sendto_obs calling s3fs_write_object_proc: path=%s, size=%zu",
                     path, bytes);
        return s3fs_write_object_proc(path, data, bytes, offset);
    } else {
        // File bucket: Use Huawei OBS write path
        // - Requires OBS headers (dentryname, inodeno)
        // - Requires IndexCache
        // - Uses OBS-specific optimizations (partial writes, server-side merge)
        // - Optimized for Huawei OBS file semantic buckets
        S3FS_PRN_DBG("[file-bucket] sendto_obs calling s3fs_write_hw_obs_proc: path=%s, size=%zu",
                     path, bytes);
...
void read_thread_task(struct HwsFdReadPage* page)
{
    ...
    if (read_size > 0)
    {
        page->read_size_ = (size_t)(read_size);
...
void whole_file_cache_task(long long inodeNo)
{
    ...
    const std::string& path = ent->GetPath();
    ...
    while ((offset < fileSize) && (g_wholeCacheTotalMemSize.load() <= g_wholeFileCacheMaxMemSize))
    {
        ...
        struct HwsFdReadPage *page = new(std::nothrow) HwsFdReadPage(path.c_str(),offset, gReadPageSize, entity_mutex, NULL);
        if (nullptr == page) {
        ...
        page->is_read_finish_ = true;
        offset += read_size;
        ent->AddWholeCachePage(page);  // wrapper: replaces wholePageList.pages_.push_back()
    ...
    /* wait to read hit */
    ent->SetWholeCacheEndTime();  // wrapper: captures monotonic timestamp
    struct timespec cacheEndTs = ent->GetWholeCacheEndTs();  // local copy for diff_in_ms
    long diffSec = 0;
    struct timespec now_ts;
    while ((diffSec < g_wholeFileCacheMaxRecycleTime
            && ent->GetWholeCacheHitCount() < g_wholeFileCacheMaxHitTimes)
            && (!ent->IsWholeWriteConflict()))
    {
        std::this_thread::sleep_for(std::chrono::seconds(READ_LOG_CONFIGURE_INTERVAL)); //10 sec
        clock_gettime(CLOCK_MONOTONIC_COARSE, &now_ts);
        diffSec = diff_in_ms(&cacheEndTs, &now_ts) / 1000;
    }
    ...
    S3FS_PRN_WARN("try to release cache pages, path(%s)inodeNo(%lld)"
                ...
                ent->GetWholeCacheHitCount(), ent->IsWholeWriteConflict());
    /* release cache data */
    std::lock_guard<std::mutex> lock(entity_mutex);
    ent->ClearWholeCache();  // wrapper: releases all pages, updates global mem counter, resets hit count
...
void setFdWholeFileCacheParasByCfg()
{
    ...
    g_wholeFileCacheNotHitSizePerPeriod = (size_t)(unsigned int)HwsGetIntConfigValue(HWS_WHOLE_CACHE_READ_SIZE) * HWS_MB_SIZE;
...
void setFdCacheGlobalVariableByCfg()
{
    ...
    gWritePageNum = (unsigned int)(HwsGetIntConfigValue(HWS_WRITE_PAGE_NUM));
    gReadPageNum = (unsigned int)(HwsGetIntConfigValue(HWS_READ_PAGE_NUM));
    S3FS_DATA_CACHE_PRN_LOG(
...
int HwsFdWritePage::Init(){
    data_ = new(std::nothrow) char[write_page_size_];
    if (nullptr == data_){
        S3FS_PRN_ERR("alloc write page fail!path(%s) offset(%ld)",
                     path_.c_str(), offset_);
        return -ENOMEM;
    }

    if (NULL == thread_task_){
        S3FS_PRN_ERR("thread task is NULL, path(%s) offset(%ld)",
                     path_.c_str(), offset_);
        return -EINVAL;
    }
    ...
    try {
        ...
        S3FS_PRN_ERR("create thread fail,path(%s)off(%ld)write_page_size(%lu)",
                     path_.c_str(), offset_, write_page_size_);
        return -EAGAIN;
    }
    return 0;
}
        ...
        const char *buf,off64_t wrtOffset, size_t wrtSize)
{
    ...
    off64_t copy_offset_in_page = wrtOffset - offset_;
    if ((size_t)(copy_offset_in_page) + wrtSize >= write_page_size_)
    {
    ...
    S3FS_PRN_INFO("new writePage,path(%s)off(%ld)write_page_size(%lu)", 
        path, offset,write_page_size);                
                ...
                if (g_test_simulate_alloc_fail > 0)
                {
                    ...
                    return nullptr;  // directly return nullptr instead of inflating size to 600GiB
                }
#endif        

    newPage = new(std::nothrow) HwsFdWritePage(path, offset, write_page_size,
        write_merge_mutex_, write_thread_task);   
    if (nullptr != newPage)
    {
        int ret = newPage->Init();
        if (ret != 0){
            delete newPage;
            newPage = nullptr;
        }
        ...
        HwsWriteLastPageRead_S *readInfo, off64_t offset, size_t readSize)
{
	...
	{
		off64_t copy_offset_in_page = readInfo->file_offset_ - page->offset_;
		readInfo->read_bytes_ = min(readSize, page->bytes_ - (size_t)(copy_offset_in_page));
...
int HwsFdReadPage::Init()
{
    data_ = new(std::nothrow) char[read_page_size_];
    if (nullptr == data_){
    ...
    try {
        ...
        S3FS_PRN_ERR("create thread fail,path(%s)off(%ld)read_page_size(%lu)",
                     path_.c_str(), offset_, read_page_size_);
        return -EAGAIN;
    }
    return 0;
}
...
bool HwsFdReadPage::CheckCrcErr(off64_t offset, size_t size)
{
    if(!gIsCheckCRC)
    {
        return false;
    ...
    int end_crc_no = (offset + (off64_t)(size) - 1)/HWS_READ_CRC_SIZE;
    int real_crc_num = (int)((read_size_ + HWS_READ_CRC_SIZE - 1)/HWS_READ_CRC_SIZE);
    for(int i = start_crc_no; i <= end_crc_no && i < real_crc_num; i++)
    {
        char *data = data_ + (int64_t)(HWS_READ_CRC_SIZE * i);
                        ...
                        off64_t offset, size_t size)
{
    ...
    for(int loop = gReadStatSize - seq_size_; loop >= 0; loop--)
    {
        ...
        {
            isSequential_ = true;
            offset_ = stat_[loop+seq_size_-1].offset + (off64_t)(stat_[loop+seq_size_-1].size);
            break;
        }
    }
...
int HwsFdReadPageList::Read(const char* path,char *buf, off64_t offset, size_t size)
{
    ...
    for(auto iter = pages_.begin(); iter != pages_.end() && copy_left_size > 0; iter++)
    {
        ...
        off64_t page_end = page->offset_ + (off64_t)(page->read_size_);
        ...
        if((copy_offset >= page->offset_) && (copy_offset < page_end))
        {
            ...
            page_copy_size = min(copy_left_size, page->read_size_ - (size_t)(copy_offset_in_page));
            if(page->CheckCrcErr(copy_offset_in_page, page_copy_size))
            {
                S3FS_PRN_ERR("hws fd cache read crc error path(%s) offset(%ld) size(%lu) read(%ld, %lu)", 
            ...
            copy_offset += (off64_t)(page_copy_size);
            copied_size += page_copy_size;
            copy_left_size -= page_copy_size;
            hit_page_num++;
            hit_file_offset = page->offset_ + copy_offset_in_page + (off64_t)(page_copy_size);
...
bool HwsFdReadPageList::PageNeedFree(HwsFdReadPage* page)
{
    ...
    unsigned int hitPercent = page->hitSizeInPage * 100 / dataSize;
    if (hitPercent > 90 && lastReadOffset > (pageEnd + HWS_MB_SIZE))
    {
        S3FS_DATA_CACHE_PRN_LOG(
          "free read page,path(%s)off(%ld)hitPer(%u),lastRead(%ld)pgEnd(%ld)", 
    ...
    S3FS_DATA_CACHE_PRN_LOG(
      ...
      "diffRecv(%ld)diffLast(%ld)thres(%ld)hitMax(%ld)hitSz(%lu)", 
            ...
            HwsFdWritePageList& writePage,off64_t fileSize)
{
    ...
    struct HwsFdReadPage *newPage = new(std::nothrow) HwsFdReadPage(path,
                                      readAheadOffset, read_page_size_,
                                      read_ahead_mutex_, read_thread_task);
    if (nullptr == newPage) {
        S3FS_PRN_ERR("alloc read page fail!path(%s) offset(%ld)",path, offset);
        return;
    }
    int ret = newPage->Init();
    if (ret != 0){
...
{
    off64_t end = offset + (off64_t)(size);
    for(auto iter = pages_.begin(); iter != pages_.end(); iter++)
    {
        HwsFdReadPage* page = *iter;
...
int HwsFdEntity::Write(const char* buf, off64_t offset, size_t size, const char* path)
{
    ...
    {
        res = (int)(AppendToPageWithRetry(path_.c_str(),buf,offset,size,entity_lock));
...
bool HwsFdEntity::HasPendingWrites()
{
    std::lock_guard<std::mutex> lock(entity_mutex_);
    return writePage_.GetCurWrtPageNum() > 0;
}
...
int HwsFdEntity::Read(char* buf, off64_t offset, size_t size, const char* path)
{
    ...
    {
        free(writePageLastReadStru.buffer_);  // fix: free buffer on error path to prevent 128KB leak per failure
    ...
    if (writePageLastReadStru.read_bytes_ > 0 )
    {
        ...
        /*read from obs*/
		ssize_t read_obs_size = 0;
		if (writePageLastReadStru.read_bytes_ < size)
		{
			read_obs_size = recvfrom_obs(path_.c_str(), buf, size, offset);
			if (read_obs_size < 0) {
				S3FS_PRN_ERR("hws fd cache read from obs fail, path(%s),offset(%ld),size(%zd)", path_.c_str(),offset, read_obs_size);
				free(writePageLastReadStru.buffer_);
				return read_obs_size;
            }
			S3FS_PRN_DBG("hws fd cache read from obs sucess,path(%s),offset(%ld),size(%zd)", 
        ...
        free(writePageLastReadStru.buffer_);
        return max(writePageLastReadStru.read_bytes_, (size_t)(read_obs_size));
    }
    /*free writePageReadStru buf*/
    free(writePageLastReadStru.buffer_);
...
std::shared_ptr<HwsFdEntity> HwsFdManager::Open(const char* path, const uint64_t inodeNo, const off64_t fileSize)
{
    ...
    {
        // Entity already exists - update file size if it changed (e.g., after truncate)
        // This handles the case where a file is reopened with O_TRUNC
        search->second->UpdateFileSizeForTruncate(fileSize);
    }
...
// Get or create FdEntity by path (for object semantic mode with virtual inodes)
std::shared_ptr<HwsFdEntity> HwsFdManager::GetByPath(const std::string& path)
{
    // For file semantic mode, use inode from cache
    if (g_bucket_type == BUCKET_TYPE_FILE_SEMANTIC) {
        tag_index_cache_entry_t entry;
        if (IndexCache::getIndexCache()->GetIndex(path, &entry)) {
            return Get(entry.inodeNo);
        }
        return nullptr;
    }

    // For object semantic mode, generate virtual inode and lookup
    if (g_bucket_type == BUCKET_TYPE_OBJECT_SEMANTIC) {
        // Generate virtual inode from path
        ino_t virtual_ino = generate_virtual_inode(path.c_str());
        return Get(virtual_ino);
    }
...
std::vector<std::shared_ptr<HwsFdEntity>> HwsFdManager::GetAllEntities()
{
    std::vector<std::shared_ptr<HwsFdEntity>> entities;

    std::lock_guard<std::mutex> lock(mutex_);

    S3FS_PRN_DBG("[HwsFdManager] Getting all open entities (count=%zu)",
                 fent_.size());

    for(auto it = fent_.begin(); it != fent_.end(); ++it){
        if(it->second){
            entities.push_back(it->second);
            S3FS_PRN_DBG("[HwsFdManager] Found open entity: inode=%llu, path=%s",
                         (unsigned long long)it->first,
                         it->second->GetPath().c_str());
        }
    }

    S3FS_PRN_INFO("[HwsFdManager] Returning %zu open entities", entities.size());
    return entities;
}

std::vector<std::shared_ptr<HwsFdEntity>> HwsFdManager::GetAllEntitiesUnderPath(const std::string& path)
{
    std::vector<std::shared_ptr<HwsFdEntity>> entities;

    if(path.empty() || path[0] != '/'){
        S3FS_PRN_ERR("Invalid path for GetAllEntitiesUnderPath: %s", path.c_str());
        return entities;
    }

    // Normalize path to ensure it ends with '/' for proper prefix matching
    ...
    S3FS_PRN_DBG("[HwsFdManager] Searching for entities under path=%s (normalized=%s)",
                 path.c_str(), search_path.c_str());

    for(auto it = fent_.begin(); it != fent_.end(); ++it){
        if(it->second){
            std::string entity_path = it->second->GetPath();

            // Check if entity_path starts with search_path
            if(entity_path.size() >= search_path.size() &&
               entity_path.compare(0, search_path.size(), search_path) == 0){

                entities.push_back(it->second);
                S3FS_PRN_INFO("[object-bucket] Found open file under %s: %s",
                            path.c_str(), entity_path.c_str());
            }
        }
    }

    S3FS_PRN_INFO("[HwsFdManager] Found %zu open files under %s",
...
unsigned int HwsFdManager::GetWritePageNumByCacheSize()
{
    ...
    off64_t pageNum = (off64_t)(gWritePageNum) * freeCacheSize/gMaxCacheMemSize;
...
unsigned int HwsFdManager::GetReadPageNumByCacheSize()
{
    ...
    off64_t pageNum = (off64_t)(gReadPageNum) * freeCacheSize/gMaxCacheMemSize;
...
void HwsFdManager::CheckCacheMemSize()       
{
    ...
    {
        ent = iter->second;
        restaticSize += (off64_t)(ent->CleanAndGetPageUsedMem());        
    }
